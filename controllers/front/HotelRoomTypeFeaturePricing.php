<?php
    class HotelRoomTypeFeaturePricing extends ObjectModel
    {
         ...
         */
        public function checkRoomTypeFeaturePriceExistance($id_product, $date_from, $date_to, $type='date_range', $current_Special_days=false, $id_feature_price=0)
        {
            if ($type == 'specific_date') {
                return Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'htl_room_type_feature_pricing` WHERE `id_product`='.(int) $id_product.' AND `date_selection_type`=2 AND `date_from` = \''.$date_from.'\' AND `id`!='.(int)$id_feature_price);

            } else if ($type == 'special_day') {
                $featurePrice = Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'htl_room_type_feature_pricing` WHERE `id_product`='.(int) $id_product.' AND `is_special_days_exists`=1 AND `active`=1 AND `date_from` < \''.$date_to.'\' AND `date_to` > \''.$date_from.'\' AND `id`!='.(int)$id_feature_price);
                if ($featurePrice) {
                ...
                return Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'htl_room_type_feature_pricing` WHERE `id_product`='.(int) $id_product.' AND `date_selection_type`=1 AND `is_special_days_exists`=0 AND `date_from` < \''.$date_to.'\' AND `date_to` > \''.$date_from.'\' AND `id`!='.(int)$id_feature_price);
            }
        }
        ...
        public function getHotelRoomTypesRatesAndInventoryByDate($id_hotel, $id_product=0, $date_from, $date_to)
        {
            ...
            for ($date = strtotime($date_from); $date <= strtotime($date_to); $date = ($date + $totalDaySeconds)) {
                ...
                if ($id_product) {
                    $roomTypeAvailabilityInfo = $objBookingDetail->DataForFrontSearch($currentDate, $nextDayDate, $id_hotel, $id_product, 1, 0, 0, -1, 0, 0);
                    ...
                    if ($hotelRoomTypes) {
                        ...
                        foreach ($hotelRoomTypes as $key => $product) {
                            ...
                            $roomTypeAvailabilityInfo = $objBookingDetail->DataForFrontSearch($currentDate, $nextDayDate, $id_hotel, $product['id_product'], 1, 0, 0, -1, 0, 0);
                            if (isset($roomTypeAvailabilityInfo['stats']['num_avail'])) {
                                $totalAvailableRooms = $roomTypeAvailabilityInfo['stats']['num_avail'];
                            } else {
                                $totalAvailableRooms = 0;
                        ...
                        if ($roomTypeRatesData['roomType']) {
                            foreach ($roomTypeRatesData['roomType'] as $key => $roomTypeRates) {
                                ...
                                if (isset($roomTypeRates['inventory'])) {
                                    ...
                                    if ($id_hotel) {
                                        $roomTypeAvailabilityInfo = $hotelBookingDetail->DataForFrontSearch($dateFrom, $dateTo, $id_hotel, $id_product, 1, 0, 0, -1, 0, 0);
                                        $bookedRoomsInfo = $hotelRoomInformation->getRoomTypeBookedRoomsForDateRange($id_hotel, $id_product, $dateFrom, $dateTo);
                                        ...
                                        if ($roomTypeRates['inventory'] <= $totalAvailableNotBooked) {
                                            if ($roomTypeRates['inventory'] < $totalAvailableRooms) {
                                                ...
                                                $availableRooms = $hotelRoomInformation->getRoomTypeAvailableRoomsForDateRange($id_hotel, $id_product, $dateFrom, $dateTo);
                                                if ($availableRooms) {
                                                    foreach ($availableRooms as $room_k => $room) {
                                                        ...
                                                        if ($hotelRoomInformation->id_status == 3) {
                                                            $disabledDates = Tools::jsonDecode($hotelRoomInformation->disabled_dates, true);
                                                            for ($date = strtotime($dateFrom); $date <= strtotime($dateTo)-$totalDaySeconds; $date = ($date + $totalDaySeconds)) {
                                                                $currentDate = date('Y-m-d', $date);
                                                                /*if ($numDisabledRooms > 0) {
                                                                    if (isset($disabledDates[$currentDate])) {
                                                                        unset($disabledDates[$currentDate]);
                                                                    }
                                                                }*/
                                                                $disabledDates[$currentDate]['reason'] = 'Disabled from channel manager.';
                                                            }
                                                        ...
                                                        $hotelRoomInformation->disabled_dates = Tools::jsonEncode($disabledDates);
                                                        $hotelRoomInformation->save();
                                                        $numDisabledRooms--;
                                                    }
                                                }
                                            } else if ($roomTypeRates['inventory'] >= $totalAvailableRooms) {
                                                $roomsToEnable = $roomTypeRates['inventory'] - $totalAvailableRooms;
                                                $disabledRooms = $hotelRoomInformation->getRoomTypeDisabledRoomsForDateRange($id_hotel, $id_product, $dateFrom, $dateTo);
                                                if ($disabledRooms) {
                                    ...
                                    $this->errors[] = $this->moduleInstance->l('Requested rooms inventory is not available.');
                                }
                            }
                        } else {
        ...
        public static function getRoomTypeTotalPrice($id_product, $date_from, $date_to)
        {
            ...
            for ($date = strtotime($date_from); $date <= (strtotime($date_to)-$totalDaySeconds); $date = ($date+$totalDaySeconds)) {
                $currentDate = date('Y-m-d', $date);
                $featurePrice = $hotelCartBookingData->getProductFeaturePricePlanByDateByPriority($id_product, $currentDate);
                if ($featurePrice) {
         ...
         * @return [float]             [returns per day feature price od the Room Type]
         */
        public static function getRoomTypeFeaturePricesPerDay($id_product, $date_from, $date_to, $use_tax=true)
        {
            ...
            $totalDurationPriceTE = $totalDurationPrice['total_price_tax_incl'];
            $totalDurationPriceTI = $totalDurationPrice['total_price_tax_excl'];
