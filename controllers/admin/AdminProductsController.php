        ...
        if (is_array($roomsInfo) && count($roomsInfo)) {
            ...
            foreach ($roomsInfo as $key => $roomInfo) {
                ...
                if (!$roomInfo['room_num']) {
                    $this->errors[] = sprintf(Tools::displayError('Please enter room number for %s room.'), $index);
                }

                if (!$roomInfo['floor']) {
                    $this->errors[] = sprintf(Tools::displayError('Please enter floor for %s room.'), $index);
                }
