...
class AdminStatsControllerCore extends AdminStatsTabController
{
    ...
    public static function getTotalSales($date_from, $date_to, $granularity = false, $id_hotel = false)
    {
        if ($granularity == 'day') {
            ...
            return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
                ...
                LEFT JOIN `'._DB_PREFIX_.'order_state` os ON o.current_state = os.id_order_state
                LEFT JOIN (
                    SELECT hbd.`id_hotel` as id_hotel, hbd.`id_order` as id_order
                    FROM`'._DB_PREFIX_.'htl_booking_detail` hbd
    ...
    public function getLatestKpiValue($kpi, $idHotels)
    {
        ...
        switch ($kpi) {
            ...
            case 'vacant_rooms':
                ...
                $totalRooms = AdminStatsController::getTotalRooms($idHotels);
            ...
            case 'total_banned_customers':
                ...
                break;
            case 'total_sales':
                $totalSales = AdminStatsController::getTotalSales('', '', false, $idHotels);
                if ($totalSales > 0) {
            ...
            case 'average_lead_time':
                $dateToday = date('Y-m-d');
                $value = Tools::ps_round(AdminStatsController::getAverageLeadTime('', '', $idHotels), 2);
                if ($value && $value <= 1) {
                    $value .= ' '.$this->l('day');
                } else {
                    $value .= ' '.$this->l('days');
    ...
    public static function getArrivalsByDate($date, $idHotel = false)
    {
        $totalArrivals = Db::getInstance()->getValue(
            ...
            FROM `'._DB_PREFIX_.'htl_booking_detail` hbd
            WHERE hbd.`id` NOT IN ('.OrderReturn::getRefundedBookingIdsSubquery().') AND hbd.`is_back_order` = 0
    ...
    public static function getDeparturesByDate($date, $idHotel = false)
    {
        $totalDepartures = Db::getInstance()->getValue(
            'SELECT COUNT(hbd.`id_room`)
            FROM `'._DB_PREFIX_.'htl_booking_detail` hbd
            WHERE hbd.`id` NOT IN ('.OrderReturn::getRefundedBookingIdsSubquery().')
    ...
    public static function getOccupancyData($dateFrom, $dateTo, $idsHotel = false)
    {
        ...
        $countRooms = (int)self::getTotalRooms($idsHotel, true);
        $countTotal = $countRooms * $numNights;
        ...
        $occupancyData['count_available'] = $countAvailable > 0 ? $countAvailable : 0;
    ...
    public static function getNightsStayed($dateFrom, $dateTo, $idHotel = false)
    {
        ...
        return Db::getInstance()->getValue(
            ...
            FROM `'._DB_PREFIX_.'htl_booking_detail` hbd
            WHERE hbd.`id` NOT IN ('.OrderReturn::getRefundedBookingIdsSubquery().') AND hbd.`is_back_order` = 0 AND
            (IF (hbd.`id_status` = '.(int) HotelBookingDetail::STATUS_CHECKED_OUT.',
    ...
    public static function getArrivalsInfoByDate($date, $idHotel = null)
    {
        ...
        WHERE hbd.`id` NOT IN ('.OrderReturn::getRefundedBookingIdsSubquery().') AND hbd.`date_from` = "'.pSQL($date).' 00:00:00"
        AND hbd.`id_status` != '.(int) HotelBookingDetail::STATUS_CHECKED_IN.'
        AND hbd.`id_status` != '.(int) HotelBookingDetail::STATUS_CHECKED_OUT.
    ...
    public static function getDeparturesInfoByDate($date, $idHotel = null)
    {
        ...
        LEFT JOIN `'._DB_PREFIX_.'orders` o ON (o.`id_order` = hbd.`id_order`)
        LEFT JOIN `'._DB_PREFIX_.'customer` c ON (c.`id_customer` = hbd.`id_customer`)
        WHERE hbd.`id` NOT IN ('.OrderReturn::getRefundedBookingIdsSubquery().') AND hbd.`date_to` = "'.pSQL($date).' 00:00:00"
        AND hbd.`id_status` = '.(int) HotelBookingDetail::STATUS_CHECKED_IN.
        (!is_null($idHotel) ? HotelBranchInformation::addHotelRestriction($idHotel, 'hbd') : '');
        $result = Db::getInstance()->executeS($sql);
    ...
    public static function getCancellationsInfoByDate($date, $idHotel = null, $eventType = null)
    {
        ...
        (!is_null($eventType) ? ' AND orr.`event_type` = '.(int) $eventType : '').
        (!is_null($idHotel) ? HotelBranchInformation::addHotelRestriction($idHotel, 'hbd') : '').'
    ...
    public static function getDistinctRoomBookingsCount($dateFrom = false, $dateTo = false, $idHotel = null, $roomBookingStatus = null)
    {
        ...
        WHERE p.`active` = 1
        AND hbd.`id` NOT IN ('.OrderReturn::getRefundedBookingIdsSubquery().')
        AND hbd.`id_status` != '.HotelBookingDetail::STATUS_CANCELLED.' '.
    ...
    {
        $result = array();
        $servicesRevenueByDates = self::getServicesRevenueForDiscreteDates($dateFrom, $dateTo, $idHotel);
        if ($roomsRevenueByDates = self::getRoomsRevenueForDiscreteDates($dateFrom, $dateTo, $idHotel)) {
            ...
            if (!Cache::isStored($cacheKey) || !$useCache) {
                ...
                if (!$onlyRooms) {
                    ...
                    IFNULL(SUM(
                        CASE
                            WHEN od.`purchase_supplier_price` <> "0.000000"
                            THEN (od.`purchase_supplier_price` / (IF (od.`product_price_calculation_method` = '.Product::PRICE_CALCULATION_METHOD_PER_BOOKING.', DATEDIFF(hbd.`date_to`, hbd.`date_from`), 1)))
                            WHEN od.`purchase_supplier_price` = "0.000000"
