...
{block name='product_errors_block'}
		...
		<div class="product_wrapper" itemscope itemtype="http://schema.org/Product">
			{if isset($product) && $product}
					...
					<div class="primary_block row">
						...
						{block name='product_left_column'}
							...
							<div class="pb-left-column col-xs-12 col-sm-8 col-md-8">
								{block name='room_type_img_containter'}
									<div class="room_type_img_containter card">
										{block name='room_hotel_name_block'}
											...
											<div class="row">
												...
												<div class="col-xs-12 col-sm-3 col-sm-pull-9 col-md-2 col-md-pull-10">
													{if isset($images) && count($images) > 0}
														<!-- thumbnails -->
															{block name='views_block'}
																<div id="views_block" class="clearfix {if isset($images) && count($images) < 2}hidden{/if}">
																	{block name='view_scroll_left'}
																		...
																		{/if}
																	{/block}
																	{block name='thumbs_list'}
																		<div id="thumbs_list">
																			{block name='thumbs_list_frame'}
																				<ul id="thumbs_list_frame">
																				{if isset($images)}
																					{foreach from=$images item=image name=thumbnails}
																						{block name='room_type_thumbnail'}
								...
								{block name='service_products_cont'}
								...
								{block name='product_info_container'}
									<div class="product_info_containter">
										<!-- tab hook is added here -->
										<!--HOOK_PRODUCT_TAB -->
										{block name='product_tabs'}
											<section class="page-product-box">
												{block name='product_description_tabs'}
													<ul class="nav nav-tabs product_description_tabs">
														...
														{/if}
														{block name='hook_product_tab'}
															{$HOOK_PRODUCT_TAB}
														{/block}
													</ul>
												{/block}
												{block name='product_description_tabs_contents'}
													<div class="tab-content product_description_tabs_contents">
														{block name='product_info_tab_cont'}
															<div id="product_info_tab" class="tab-pane active card">
																{block name='product_info_tab_information'}
														...
														{/block}
														{* Block for booking products *}
														{block name='refund_policies_tab_cont'}
															{if isset($id_hotel) && $id_hotel}
															...
															{/if}
														{/block}
														{block name='room_type_map_tab_cont'}
															{if $display_google_maps && ($hotel_latitude|floatval != 0 && $hotel_longitude|floatval != 0)}
				...
				{if !$content_only}
					...
					{/block}
					{block name='blockpack'}
						{if isset($packItems) && $packItems|@count > 0}
							<section id="blockpack">
							<h3 class="page-product-heading">{l s='Pack content'}</h3>
							{block name='product_list_container'}
					...
					{block name='product_accessories'}
						...
						{/if}
					{/block}
					{block name='hook_product_product_footer'}
						{if isset($HOOK_PRODUCT_FOOTER) && $HOOK_PRODUCT_FOOTER}{$HOOK_PRODUCT_FOOTER}{/if}
					{/block}
					<!-- description & features -->
					{if (isset($product) && $product->description) || (isset($features) && $features) || (isset($accessories) && $accessories) || (isset($HOOK_PRODUCT_TAB) && $HOOK_PRODUCT_TAB) || (isset($attachments) && $attachments) || isset($product) && $product->customizable}
						{block name='product_download_attachments'}
										...
										<div class="customizableProductsFile">
											...
											<ul id="uploadable_files" class="clearfix">
												...
												{foreach from=$customizationFields item='field' name='customizationFields'}
													{block name='product_customization_upload_field'}
														{if $field.type == 0}
