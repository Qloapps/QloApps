...
{block name='discount_content'}
	{capture name=path}<a href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">{l s='My account'}</a><span class="navigation-pipe">{$navigationPipe}</span><span class="navigation_page">{l s='Vouchers'}</span>{/capture}
		...
		{block name='discount_page_heading'}
			<h1 class="page-heading">
				{l s='Vouchers'}
			</h1>
		{/block}

	{block name='discount_page_content'}
		{if isset($cart_rules) && $nb_cart_rules}
			{block name='discounts_detail_table'}
				<table class="discount table table-bordered footab">
					{block name='discounts_detail_table_head'}
						<thead>
							<tr>
					...
					{/block}
					{block name='discounts_detail_table_body'}
						<tbody>
							{foreach from=$cart_rules item=discountDetail name=myLoop}
								{block name='discounts_detail_table_row'}
			...
			{/block}
		{else}
			{block name='discount_page_error'}
				<p class="alert alert-warning">{l s='You do not have any vouchers.'}</p>
			{/block}
		{/if}
	{/block}

	{block name='discount_page_footer_links'}
