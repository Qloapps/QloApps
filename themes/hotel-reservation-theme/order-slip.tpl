...
{block name='order_slip'}
	{capture name=path}
		...
		<li class="breadcrumb-item"><span class="navigation_page">{l s='Credit slips'}</span></li>
	{/capture}
	...
	<div class="container-md qlo-mobile-reponsive-table">
		<div class="d-flex justify-content-center">
			<div class="qlo-account-card card col-12 p-5 mb-md-4">
				<h1 class="page-heading text-center text-bolder h5 font-weight-bold">{l s='Credit Slips'}</h1>
				<hr>
				<p class="text-center small mb-4">{l s='Credit slips you have received after canceled orders'}.</p>
					...
					<div class="block-center" id="block-history">
						{if $ordersSlip && count($ordersSlip)}
							<div class="table-responsive wk-datatable-wrapper">
								<table id="order-list" class="table table-borderd table-striped-blue footab">
									<thead>
										<tr>
									...
									<tbody>
										{foreach from=$ordersSlip item=slip name=myLoop}
											<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
												...
												<td class="bold" data-value="{$slip.date_add|regex_replace:"/[\-\:\ ]/":""}" data-label="{l s='Date issued'}">
													{dateFormat date=$slip.date_add full=0}
												</td>
												<td class="history_invoice" data-label="{l s='View credit slip'}">
