					...
					<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="login_form" class="box">
						...
						<div class="mt-5">
							<div class="form_content">
								...
								{/block}
								<p class="lost_password form-group"><a class="text-primary" href="{$link->getPageLink('password')|escape:'html':'UTF-8'}" title="{l s='Recover your forgotten password'}" rel="nofollow">{l s='Forgot your password?'}</a></p>
								{block name='authentication_login_submit'}
									<div class="submit">
										{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'html':'UTF-8'}" />{/if}
										<button type="submit" id="SubmitLogin" name="SubmitLogin" class="btn btn-primary col-12">
											<span>{l s='Login'}</span>
										</button>
									</div>
								{/block}
								{block name='authentication_register_link_block'}
									<div class="text-center mt-4 mb-1">
										{l s='Don’t have an account?'} <a class="text-primary" href="{$link->getPageLink('register')}">{l s='Sign Up'}</a>
									</div>
								{/block}
							...
							<div class="cleafix gender-line">
								...
								{foreach from=$genders key=k item=gender}
									<div class="form-check">
										<input type="radio" name="id_gender" id="id_gender{$gender->id}" value="{$gender->id}"{if isset($smarty.post.id_gender) && $smarty.post.id_gender == $gender->id} checked="checked"{/if}  class="form-check-input"/>
										<label for="id_gender{$gender->id}" class="form-check-label">
											{$gender->name}
										</label>
									</div>
								{/foreach}
