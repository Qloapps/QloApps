...
{block name='address'}
	{capture name=path}
		...
		<li class="breadcrumb-item"><span class="navigation_page">{l s='My addresses'}</span></li>
	{/capture}
	<div class="container-md">
		{if isset($smarty.get.updated) && $smarty.get.updated}
			<p class="alert alert-success">{l s='Your address has been successfully updated.'}</p>
		{/if}
		<div class="row m-md-4 justify-content-center">
			<div class="qlo-account-card card col-12 col-md-9 p-5 mb-md-4">
				{block name='address_heading'}
					<h1 class="page-heading text-center text-bolder h5 font-weight-bold">{l s='My Addresses'}</h1>
				{/block}
				<hr>
				...
				{block name='address_form'}
					<form action="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" method="post" class="std qlo-address-form" id="add_address">
						...
						{foreach from=$ordered_adr_fields item=field_name}
							...
							{if $field_name eq 'phone'}
								...
								{if isset($one_phone_at_least) && $one_phone_at_least}
									...
									<p class="float-left text-danger required">** {l s='You must register at least one phone number.'}</p>
								{/if}
						...
						<p class="submit2 qlo-address-submit">
							{if isset($id_address)}<input type="hidden" name="id_address" value="{$id_address|intval}" />{/if}
							{if isset($back)}<input type="hidden" name="back" value="{$back}" />{/if}
							{if isset($mod)}<input type="hidden" name="mod" value="{$mod}" />{/if}
							{if isset($select_address)}<input type="hidden" name="select_address" value="{$select_address|intval}" />{/if}
							<input type="hidden" name="token" value="{$token}" />
							<button type="submit" name="submitAddress" id="submitAddress" class="btn btn-primary btn-medium header-rmsearch-input">
								<span>
									{if isset($id_address)}{l s='Update'}{else}{l s='Save'}{/if}
								</span>
							</button>
						</p>
					</form>
