...
<div class="block-center" id="block-history">
	{if $orders && count($orders)}
		{block name='history_table'}
			<table id="order-list" class="table table-bordered footab">
				{block name='history_table_header'}
					<thead>
						<tr>
				...
				{/block}
				{block name='history_table_body'}
					<tbody>
						{foreach from=$orders item=order name=myLoop}
							{block name='history_table_row'}
								<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
									{block name='history_order_reference'}
									...
									{/block}
									{block name='history_order_due_price'}
										{if isset($adv_active)}
										...
										{/if}
									{/block}
									{block name='history_order_method'}
										<td class="history_method">{$order.payment|escape:'html':'UTF-8'}</td>
									{/block}
									{block name='history_order_state'}
									...
									{/block}
									{block name='history_order_invoice'}
									...
									{/block}
									{block name='history_order_detail_button'}
