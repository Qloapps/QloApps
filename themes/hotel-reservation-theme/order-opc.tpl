				...
				<div class="row">
					...
					{else}
						{if $productNumber}
							...
							<div class="col-md-8">
							{block name='order_opc_errors_block'}
								{include file="$tpl_dir./errors.tpl"}
							{/block}
								{* Accordian for all blocks *}
								{block name='order_opc_accordion'}
										...
										{if isset($checkout_process_steps) && $checkout_process_steps}
											{foreach $checkout_process_steps as $step}
															...
															{if $step->step_is_reachable}
																<div id="collapse-shopping-cart" class="opc-collapse {if !$step->step_is_current}collapse{/if}" aria-labelledby="shopping-cart-head" data-parent="#oprder-opc-accordion">
																	<div class="card-body">
																		...
																		{block name='order_opc_shopping_cart_container'}
												...
												{elseif $step->step_key == 'checkout_customer'}
													{block name='order_opc_guest_info'}
																	...
																	<div class="card-body">
																		{if $is_logged || $isGuest}
																			...
																			<div id="checkout-guest-info-block"  {if $id_customer_guest_detail}style="display: none;"{/if}>
																				{block name='order_opc_guest_info_name'}
																			...
																			{if !$orderRestrictErr}
																				{block name='order_opc_guest_info_actions'}
																					<hr>
																					<div class="row">
												...
												{elseif $step->step_key == 'checkout_payment'}
													...
													{block name='order_opc_payment_block'}
														<div class="card">
															...
															{if $step->step_is_reachable}
																<div id="collapse-order-payment" class="opc-collapse {if !$step->step_is_current}collapse{/if}" aria-labelledby="order-payment-head" data-parent="#oprder-opc-accordion">
																	<div class="card-body">
																		<!-- Payment -->
																		{block name='order_payment_container'}
							...
							{block name='order_opc_cart_summary_block'}
								<div class="col-md-4">
									{block name='cart_total_detail_block'}
									{* Total cart details, tax details, advance payment details and voucher details *}
										<div class="col-sm-12 card cart_total_detail_block">
											...
											{/if}
											{block name='hook_display_cart_total_price_label_before_tax'}
											...
											<p class="cart_final_total_block">
												...
												{else}
													...
													<div class="hookDisplayProductPriceBlock-price">
														{block name='hook_display_cart_total_price_label'}
									...
									{if $voucherAllowed}
										{block name='cart_voucher_detail_block'}
											{* Cart vouchers block *}
									...
									{/if}
									{* End Voucher Block *}
									{block name='hook_display_cart_right_column'}
