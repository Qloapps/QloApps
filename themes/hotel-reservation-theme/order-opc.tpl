													...
													{block name='order_opc_rooms_summary'}
														<div class="mb-3">
															{block name='order_opc_rooms_summary_heading'}
																<a class="checkout-page-cart-head {if !$step->step_is_current}collapsed{/if}" data-toggle="collapse" href="#collapse-shopping-cart" aria-expanded="{if $step->step_is_current}true{else}false{/if}">{l s='Summary'}</a>
															{/block}
														</div>
														{if $step->step_is_reachable}
															{* This tpl includes room type lists in the orders *}
															{block name='shopping_cart'}
																<div id="collapse-shopping-cart" class="opc-collapse {if !$step->step_is_current}collapse{/if}" aria-labelledby="shopping-cart-head">
													...
													{block name='order_opc_guest_information'}
														<div class="mb-3">
															{block name='order_opc_guest_information_heading'}
																<a class="checkout-page-cart-head {if !$step->step_is_current}collapsed{/if}" data-toggle="collapse" data-target="#collapse-guest-info" aria-expanded="{if $step->step_is_current}true{else}false{/if}">{l s='Guest Information'}</a>
															{/block}
														</div>
															{if $step->step_is_reachable}
																{block name='order_opc_guest_detail_wrapper'}
																	<div id="collapse-guest-info" class="opc-collapse {if !$step->step_is_current}collapse{/if}" aria-labelledby="guest-info-head">
																				...
																				{block name='order_opc_guest_detail_form'}
																					<form id="customer_guest_detail_form" method="post" action="{$link->getPageLink('order-opc', null, null)}">
																						...
																						<div id="customer-guest-detail-container" {if !$id_customer_guest_detail}style="display: none;"{/if}>
																							...
																							<div class="row">
																								<div class="required form-group col-sm-12">
																							...
																							<div class="row">
																								<div class="required text form-group col-sm-12">
																							...
																							<div class="row">
																								<div class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group col-sm-12">
																			...
																			{block name='order_opc_guest_detail'}
																				<div id="checkout-guest-info-block"  {if $id_customer_guest_detail}style="display: none;"{/if}>
																					<div class="row margin-btm-10">
																						<div class="col-sm-3 col-5 info-head">{l s='Name'}</div>
																						<div class="col-sm-9 col-7 info-value">
																							{if $isGuest}
																					...
																					<div class="row margin-btm-10">
																						<div class="col-sm-3 col-5 info-head">{l s='Email'}</div>
																						<div class="col-sm-9 col-7 info-value">{$guestInformations['email']|escape:'html':'UTF-8'}</div>
																					</div>
																					{if (isset({$guestInformations['phone']}) && {$guestInformations['phone']})}
																						<div class="row margin-btm-10">
																							<div class="col-sm-3 col-5 info-head">
																								{l s='Phone Number'}
																							</div>
																							<div class="col-sm-9 col-7 info-value">
																				...
																				{block name='order_opc_guest_detail_proceed_action'}
																					...
																					<div class="row">
																						<div class="col-sm-12 proceed_btn_block">
																							<a class="btn btn-primary btn-medium pull-right submit-guest-details pull-right" href="{$link->getPageLink('order-opc', null, null, ['proceed_to_payment' => 1])}" title="Proceed to Payment" rel="nofollow">
																								<span>
																									{l s='Proceed'}
																								</span>
																							</a>
																		...
																		{/if}
																		{if $is_logged || $isGuest}
																			<div class="hidden" id="order-opc-edit-guest-info">
													...
													{block name='order_opc_payment'}
														<div class="mb-3">
															{block name='order_opc_payment_heading'}
																<a class="checkout-page-cart-head {if !$step->step_is_current}collapsed{/if}" data-toggle="collapse" data-target="#collapse-order-payment" aria-expanded="{if $step->step_is_current}true{else}false{/if}">{l s='Payment Options'}</a>
															{/block}
														</div>
														{if $step->step_is_reachable}
															<div id="collapse-order-payment" class="opc-collapse {if !$step->step_is_current}collapse{/if}" aria-labelledby="order-payment-head">
																<!-- Payment -->
								...
								{block name='order_opc_right_column'}
									<div class="col-md-4">
										...
										{block name='order_opc_vouchers'}
											...
											{if $voucherAllowed}
												{* Cart vouchers block *}
												<div class="col-sm-12 qlo-card cart_voucher_detail_block">
													...
													<div class="row mb-4">
														...
														<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
															<div class="col-12 col-lg-7 float-left pr-0">
																<input type="text" class="discount_name form-control" id="discount_name" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" placeholder="{l s='Available Coupons'}"/>
																<input type="hidden" name="submitDiscount" />
															</div>
															<div class="col-12 col-lg-5 float-left pr-0 submit_discount_div">
																<button type="submit" name="submitAddDiscount" class="btn-low btn-primary">
																	<span>{l s='Apply'}</span>
																</button>
															</div>
														</form>
													...
													{if sizeof($discounts)}
														<div class="row">
															{foreach $discounts as $discount}
																<div class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
																	<span class="cart_discount_name voucher_txt_green">
														...
														<div class="row avail_vouchers_block">
															{foreach from=$displayVouchers key=key item=voucher name=availVoucher}
																<div class="col-12">
																	...
																	<span class="voucher_name voucher_txt_green" data-code="{$voucher.code|escape:'html':'UTF-8'}">{$voucher.code|escape:'html':'UTF-8'} -</span>&nbsp;{$voucher.name}
																	</p>
