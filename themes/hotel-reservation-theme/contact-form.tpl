...
{block name='contact_form'}
	<div class="my-5 container-md">
		...
		<div class="text-center mb-4 mb-lg-5">
			<div class="contact-header mb-2 heading h4 font-weight-bold">{l s='Contact Us'}</div>
			<div class="contact-desc text-muted">{l s='Reach out to us for any inquiries or assistance. We\'re here to help make your experience with us exceptional.'}</div>
		</div>
		<div class="row">
			<div class="col-12 col-lg-8 {if !(isset($gblHtlAddress) && $gblHtlAddress) && !(isset($gblHtlPhone) && $gblHtlPhone) && !(isset($gblHtlEmail) && $gblHtlEmail)} offset-lg-2 {/if}">
				{block name='contact_form_content'}
				{if isset($customerThread.token)}
					<form action="{$link->getPageLink('contact', null, null, array('token' => $customerThread.token))}" method="post" class="contact-form-box qlo-account-card card p-4 qlo-padding-sm" enctype="multipart/form-data">
				{else}
					<form action="{$link->getPageLink('contact')}" method="post" class="contact-form-box qlo-account-card card p-4 qlo-padding-sm" enctype="multipart/form-data">
				{/if}
					<div class="row">
						...
						{/if}
						<div class="col-12 {if isset($displayContactName) && $displayContactName}col-md-6{/if}">
							<div class="form-group mb-3">
						...
						{if !isset($customerThread.id_contact) && isset($allowContactSelection) && $allowContactSelection}
							<div class="col-12 col-md-6">
								<div class="form-group mb-3">
									...
									<div class="dropdown">
										<button class="form-control contact_type_input d-flex align-items-center justify-content-between" type="button" data-toggle="dropdown">
											<span id="contact_type">{l s='Choose'}</span>
						...
						<div class="col-12">
							<div class="form-group mb-3">
								...
								<textarea class="form-control contact_textarea" id="message" name="message" rows="6">{if isset($message)}{$message|escape:'html':'UTF-8'|stripslashes}{/if}</textarea>
							</div>
						</div>
						{if $fileupload == 1}
							<div class="col-12">
						...
						<div class="col-12">
							...
							<div class="form-group mb-0">
								...
								<button class="btn btn-primary" type="submit" name="submitMessage" id="submitMessage"><span>{l s='Send Message'}</span></button>
							</div>
						</div>
					</div>
					</form>
			...
			{if (isset($gblHtlAddress) && $gblHtlAddress) && (isset($gblHtlPhone) && $gblHtlPhone) && (isset($gblHtlEmail) && $gblHtlEmail)}
				<div class="col-12 col-lg-4 mt-4 mt-lg-0">
					{block name='contact_form_info'}
						<div class="htl-global-address-div qlo-account-card card p-4 qlo-padding-sm">
							<div class="mb-3 text-secondary small"><i class="icon-building"></i> {l s='Main Branch'}</div>
							<div class="global-address-value mb-3">{$gblHtlAddress}</div>
							<div class="mb-3">
								<div class="mb-2 text-secondary small"><i class="icon-phone"></i> {l s='Phone'}</div>
								<div class="global-address-value">{$gblHtlPhone}</div>
							</div>
							<div class="mb-3">
								<div class="mb-2 text-secondary small"><i class="icon-envelope"></i> {l s='Mail Us'}</div>
								<div class="global-address-value">{$gblHtlEmail}</div>
							</div>
							{if isset($gblHtlRegistrationNumber) && $gblHtlRegistrationNumber}
								<div class="mb-3">
									<div class="mb-2 text-secondary small"><i class="icon-book"></i> {l s='Registration number'}</div>
									<div class="global-address-value">{$gblHtlRegistrationNumber}</div>
								</div>
							{/if}
							{if isset($gblHtlFax) && $gblHtlFax}
								<div class="mb-0">
									<div class="mb-2 text-secondary small"><i class="icon-fax"></i> {l s='Fax'}</div>
		...
		{block name='contact_form_hotel_branches'}
			{if isset($displayHotels) && $displayHotels && isset($hotelsInfo) && $hotelsInfo}
				<div class="row hotels-container">
					<div class="col-12 heading h4 text-center my-5">
						<span>{l s='Our Hotels'}</span>
					</div>
					{foreach $hotelsInfo as $hotel}
						<div class="col-12 col-md-6 col-lg-4 mb-4">
							<div class="hotel-address-container card qlo-account-card h-100">
								{if $hotel['image_url']}
									<img class="card-img-top" src="{$hotel['image_url']}" alt="{$hotel['hotel_name']|escape:'html':'UTF-8'}">
								{/if}
								<div class="card-body d-flex flex-column">
									...
									{/if}
									{if $hotel['email']}
										<div class="hotel-branch-info-value mb-3 small">
											<i class="icon-regular icon-envelope"></i>{$hotel['email']}
										</div>
									{/if}
									{if ($hotel['latitude'] != 0 || $hotel['longitude'] != 0) && $viewOnMap}
										<div class="mt-auto">
											<a class="btn btn-primary btn-sm htl-map-direction-btn" href="http://maps.google.com/maps?daddr=({$hotel['latitude']},{$hotel['longitude']})" target="_blank">
		...
		{block name='contact_form_hotel_locations'}
			{if isset($displayHotelMap) && $displayHotelMap && isset($hotelLocationArray)}
				<div class="row {if !(isset($displayHotels) && $displayHotels && isset($hotelsInfo) && $hotelsInfo)} mt-4{/if}">
					<div class="col-12" id="googleMapWrapper">
						<div class="card qlo-account-card overflow-hidden">
							<div id="map" class="w-100" style="min-height: 320px;"></div>
						</div>
					</div>
				</div>
			{/if}
