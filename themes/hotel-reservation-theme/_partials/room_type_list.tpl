												...
												{if (!isset($display_all_room_types) || !$display_all_room_types)}
													<div class="booking_room_fields">
														...
														{block name='room_type_list_room_book_now_button'}
															<div>
																<a cat_rm_check_in="{$booking_date_from|escape:'htmlall':'UTF-8'}" cat_rm_check_out="{$booking_date_to|escape:'htmlall':'UTF-8'}" href="" rm_product_id="{$room_v['id_product']}" cat_rm_book_nm_days="{$num_days|escape:'htmlall':'UTF-8'}" data-id-product-attribute="0" data-id-product="{$room_v['id_product']|intval}" class="qlo-btn qlo-btn--primary ajax_add_to_cart_button"><span>{l s='Book Now'}</span></a>
															</div>
														{/block}
													</div>
												{else}
