    ...
    {block name='service_products_tabs'}
        <ul class="nav nav-pills">
            {if !$PS_SERVICE_PRODUCT_CATEGORY_FILTER}
                <li class="nav-item"><a href="#all_products" class="idTabHrefShort nav-pill active">{l s='Services'}</a></li>
            {else}
                {foreach $service_products_by_category as $category}
                    <li class="nav-item"><a class="idTabHrefShort nav-pill {if $category@iteration == 1}active{/if}"  href="#category_{$category['id_category']}" data-toggle="tab">{$category['name']}</a></li>
                {/foreach}
            {/if}
        </ul>
    {/block}
