    ...
    public static function getRegistrationCardInfo()
    {
        $guestRegCardInfo = array(
            self::GRC_SECTION_GUEST_INFO => array(
                ...
                'fields' => array(
                    self::GRC_GUEST_TITLE        => 'Title',
                    self::GRC_GUEST_FULL_NAME    => 'Full Name (As per ID)',
                    self::GRC_GUEST_PHONE        => 'Phone / Mobile',
                    self::GRC_GUEST_EMAIL        => 'Email',
                    self::GRC_GUEST_DOB          => 'Date of Birth',
                    self::GRC_GUEST_NATIONALITY  => 'Nationality',
                    self::GRC_GUEST_CITY_COUNTRY => 'City / Country',
                    self::GRC_GUEST_POSTAL_CODE  => 'Postal Code',
                    self::GRC_GUEST_ADDRESS      => 'Address',
                ),
            ),
            self::GRC_SECTION_TRAVEL_INFO => array(
                'name'   => 'Travel Information',
                'fields' => array(
                    self::GRC_TRAVEL_ARRIVED_FROM     => 'Arrived From',
                    self::GRC_TRAVEL_NEXT_DESTINATION => 'Next Destination',
                    self::GRC_TRAVEL_FLIGHT_TRAIN     => 'Flight / Train Number',
                    self::GRC_TRAVEL_VEHICLE_REG      => 'Vehicle Reg. No.',
                    self::GRC_TRAVEL_PURPOSE_OF_VISIT => 'Purpose of Visit',
                ),
            ),
            self::GRC_SECTION_BOOKING_INFO => array(
                'name'   => 'Booking Information',
                'fields' => array(
                    self::GRC_BOOKING_REFERENCE      => 'Booking Reference No.',
                    self::GRC_BOOKING_RATE_PER_NIGHT => 'Rate per Night',
                    self::GRC_BOOKING_ARRIVAL        => 'Arrival Date & Time',
                    self::GRC_BOOKING_DEPARTURE      => 'Departure Date & Time',
                    self::GRC_BOOKING_ROOM_TYPE      => 'Room Type',
                    self::GRC_BOOKING_ROOM_NUMBER    => 'Room Number',
                    self::GRC_BOOKING_NUM_GUESTS     => 'Number of Guests',
                ),
            ),
            self::GRC_SECTION_IDENTIFICATION => array(
                'name'   => 'Identification Document',
                'fields' => array(
                    self::GRC_ID_IDENTITY_PROOF          => 'Identity Proof',
                    self::GRC_ID_NUMBER                  => 'ID Number',
                    self::GRC_ID_PASSPORT_NO             => 'Passport No.',
                    self::GRC_ID_PLACE_OF_ISSUE          => 'Place of Issue',
                    self::GRC_ID_DATE_OF_ISSUE           => 'Date of Issue',
                    self::GRC_ID_DATE_OF_EXPIRY          => 'Date of Expiry',
                    self::GRC_ID_VISA_NUMBER             => 'Visa Number',
                    self::GRC_ID_VALID_UNTIL             => 'Valid Until',
                    self::GRC_ID_ARRIVAL_DATE_IN_COUNTRY => 'Arrival Date in Country',
                ),
            ),
            self::GRC_SECTION_ADDITIONAL_GUESTS => array(
                'name'   => 'Additional Guests',
                'fields' => array(
                    self::GRC_ADD_GUEST_NAME        => 'Guest Name',
                    self::GRC_ADD_GUEST_ID_TYPE     => 'ID Type',
                    self::GRC_ADD_GUEST_ID_NUMBER   => 'ID Number',
                    self::GRC_ADD_GUEST_NATIONALITY => 'Nationality',
                ),
            ),
            self::GRC_SECTION_BILLING_CORPORATE => array(
                'name'   => 'Billing & Corporate Details',
                'fields' => array(
                    self::GRC_BILLING_COMPANY => 'Company / Agent',
                    self::GRC_BILLING_TAX_ID  => 'Tax ID / VAT No.',
                ),
            ),
            self::GRC_SECTION_PAYMENT_DEPOSIT => array(
                'name'   => 'Payment & Deposit',
                'fields' => array(
                    self::GRC_PAYMENT_METHOD           => 'Payment Method',
                    self::GRC_PAYMENT_CARD_NUMBER      => 'Credit Card Number',
                    self::GRC_PAYMENT_SECURITY_DEPOSIT => 'Security Deposit',
                ),
            ),
            self::GRC_SECTION_GUEST_SIGNATURE => array(
                'name'   => 'Guest Signature',
                'fields' => array(
                    self::GRC_SIG_SIGNATURE => 'Signature',
                    self::GRC_SIG_DATE      => 'Date',
                ),
            ),
            self::GRC_SECTION_PROPERTY_REGS => array(
                'name'   => 'Property Regulations',
                'fields' => array(
                    self::GRC_PROP_CHECKIN_CHECKOUT_TIME => 'Check-in / Check-out Time',
                    self::GRC_PROP_HOTEL_POLICIES        => 'Hotel Policies',
                ),
            ),
            self::GRC_SECTION_OFFICE_USE_ONLY => array(
                'name'   => 'For Office Use Only',
                'fields' => array(
                    self::GRC_OFFICE_STAFF_NAME   => 'Staff Name',
                    self::GRC_OFFICE_CHECKIN_TIME => 'Check-in Time',
                    self::GRC_OFFICE_ID_VERIFIED  => 'ID Verified',
                    self::GRC_OFFICE_REG_NO       => 'Registration No.',
                ),
            ),
        );
