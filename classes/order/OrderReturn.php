...
class OrderReturnCore extends ObjectModel
{
    ...
    public function getOrderRefundRequestedBookings($idOrder, $idOrderReturn = 0, $onlyBookingIds = 0, $customerView = 0, $skipReqCompletedNonRefunded = 0)
    {
        ...
        if ($returnDetails = Db::getInstance()->executeS($sql)) {
            ...
            $onlineTourismTaxByBooking = Configuration::get('QLO_USE_TOURISM_TAX') ? HotelOrderTourismTax::getOnlineTotalsByBooking($idOrder) : array();
            ...
            foreach ($returnDetails as $key => &$bookingRow) {
                ...
                if (!$onlyBookingIds) {
                    ...
                    $bookingRow['tourism_tax_online_amount'] = 0;
                    ...
                    $tourismTaxOnline = isset($onlineTourismTaxByBooking[(int)$bookingRow['id_htl_booking']]) ? (float) $onlineTourismTaxByBooking[(int)$bookingRow['id_htl_booking']] : 0.0;
                    $bookingRow['tourism_tax_online_amount'] = $tourismTaxOnline;
                    if ($tourismTaxOnline > 0 && $objOrder->total_paid_real > 0) {
                        $denominator = $objOrder->total_paid_tax_incl + $objOrder->total_discounts_tax_incl;
                        $bookingRow['tourism_tax_paid_amount'] = $denominator > 0 ? ($objOrder->total_paid_real * $tourismTaxOnline / $denominator) : 0.0;
                    }
                    if ($customerView) {
