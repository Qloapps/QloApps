...
class AdminOrdersControllerCore extends AdminController
{
	...
	public function renderForm()
	{
		...
		{
			$cart_id = Tools::getValue('cart_id');//get cart id from url
			$htl_cart = new Cart($cart_id);
			$this->context->cart = $htl_cart;
			...
			($id_order_state = Tools::getValue('id_order_state')) && Validate::isModuleName($module_name))
		{
			if ($this->tabAccess['edit'] === '1')
			{
				...
				else
				{
					...
					{
						setcookie('id_cart', " " , time() - 86400, "/");
						setcookie('id_guest', " " , time() - 86400, "/");
	...
	public function renderView()
	{
		...
		if ($cart_detail_data)
		{
			...
			foreach ($cart_detail_data as $key => $value)
			{
				...
				$cart_detail_data[$key]['avail_rooms_to_swap'] = $obj_htl_bk_dtl->getAvailableRoomsForSwaping($value['date_from'], $value['date_to'], $value['id_product'], $value['id_hotel']);
