...
class Dashgoals extends Module
{
	...
	public function getChartData($year)
	{
		...
		if (Configuration::get('PS_DASHBOARD_SIMULATION')) {
			...
			$orders = Order::getOrderCount(['date_from' => $year.date('-01-01'), 'date_to' => $year.date('-12-31'), 'granularity' => 'month']);
			$sales = Order::getTotalSales(['date_from' => $year.date('-01-01'), 'date_to' => $year.date('-12-31'), 'granularity' => 'month']);
