...
class WkPaypalCommerceDb
{
    ...
    public function createPaypalAwaitingOrderState()
    {
        $orderStatus = new OrderState();
        $orderStatus->invoice = 0;
        $orderStatus->send_email = 0;
        $orderStatus->module_name = 'qlopaypalcommerce';
        $orderStatus->color = '#ff8942';
        $orderStatus->unremovable = 1;

        foreach (Language::getLanguages(false) as $lang) {
            $orderStatus->name[$lang['id_lang']] = 'PayPal Payment Awaiting';
            $orderStatus->template[$lang['id_lang']] = '';
        }

        if ($orderStatus->save()) {
            $source = _PS_MODULE_DIR_.'qlopaypalcommerce/views/img/1.gif';
            $destination = _PS_ORDER_STATE_IMG_DIR_.$orderStatus->id.'.gif';
            Tools::copy($source, $destination);

            Configuration::updateValue('PS_OS_PAYPAL_COMMERCE_AWAITING', $orderStatus->id);
    ...
    public function createPartialPaymentOrderState()
    {
        if (!Configuration::get('PS_OS_PARTIAL_PAYMENT')) {
            $objOrderState = new OrderState();
            $objOrderState->name = array();

            foreach (Language::getLanguages() as $language) {
                $objOrderState->name[$language['id_lang']] = 'Partial Payment Accepted';
            }
            $objOrderState->module_name = 'qlopaypalcommerce';
            $objOrderState->send_email = false;
            $objOrderState->unremovable = false;
            $objOrderState->color = '#AAFFA4';
            $objOrderState->hidden = false;
            $objOrderState->delivery = false;
            $objOrderState->logable = true;
            $objOrderState->invoice = true;

            if ($objOrderState->save()) {
                $source = _PS_MODULE_DIR_.'qlopaypalcommerce/logo.gif';
                $destination = _PS_ORDER_STATE_IMG_DIR_.$objOrderState->id.'.gif';
                Tools::copy($source, $destination);

                Configuration::updateValue('PS_OS_PARTIAL_PAYMENT', (int) $objOrderState->id);
            }
        }
