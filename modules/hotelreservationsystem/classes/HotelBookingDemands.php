...
class HotelBookingDemands extends ObjectModel
{
    ...
    /**
     * Returns total service revenue (spod + demands) for the given date range.
     *
     * Replicates: HotelAnalyticsCore::getServiceRevenue() aggregate.
     *
    ...
    public static function getTotalServiceRevenue(array $params)
    {
        ...
        $roomFilter     = $idRoom     ? ' AND hbd.`id_room` = '.$idRoom          : '';
        $orderFilter    = $idOrder    ? ' AND hbd.`id_order` = '.$idOrder        : '';
        $customerFilter = $idCustomer ? ' AND hbd.`id_customer` = '.$idCustomer  : '';

        $total = 0;

        // 1) Room-linked service products (service_product_order_detail)
        $spodValue = Db::getInstance()->getValue(
            'SELECT IFNULL(SUM(
            ...
            .$productFilter.$roomFilter.$orderFilter.$customerFilter.$hotelFilter
        );
        $total += (float) $spodValue;

        // 2) Extra demands (htl_booking_demands)
        $demandsValue = Db::getInstance()->getValue(
            'SELECT IFNULL(SUM(
        ...
        $total += (float) $demandsValue;

        return $total;
    }

    /**
     * Returns per-day service revenue keyed by Unix timestamp.
     *
     * @param array $params date_from, date_to, id_hotel, id_product, id_room, id_order, id_customer
    ...
    public static function getDatewiseServiceRevenue(array $params)
    {
        ...
        while ($current <= $dateTo) {
            ...
            $cacheKey = 'HotelBookingDemands::getDatewiseServiceRevenue_'.$ts.'_'
            ...
            if (!Cache::isStored($cacheKey)) {
                ...
                Cache::store($cacheKey, (float) $spodValue + (float) $demandsValue);
            }
