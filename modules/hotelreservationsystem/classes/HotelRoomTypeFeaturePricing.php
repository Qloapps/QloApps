    ...
    class HotelRoomTypeFeaturePricing extends ObjectModel
    {
        ...
        public function checkRoomTypeFeaturePriceExistance($id_product, $date_from, $date_to, $type='date_range', $current_Special_days=false, $id_feature_price=0)
        {
            if ($type == 'specific_date') {
                return Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'htl_room_type_feature_pricing` WHERE `id_product`='.(int) $id_product.' AND `date_selection_type`=2 AND `date_from` = \''.pSQL($date_from).'\' AND `id`!='.(int) $id_feature_price);
            } elseif ($type == 'special_day') {
                $featurePrice = Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'htl_room_type_feature_pricing` WHERE `id_product`='.(int) $id_product.' AND `is_special_days_exists`=1 AND `active`=1 AND `date_from` < \''.pSQL($date_to).'\' AND `date_to` > \''.pSQL($date_from).'\' AND `id`!='.(int) $id_feature_price);
                if ($featurePrice) {
                ...
                return Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'htl_room_type_feature_pricing` WHERE `id_product`='.(int) $id_product.' AND `date_selection_type`=1 AND `is_special_days_exists`=0 AND `date_from` < \''.pSQL($date_to).'\' AND `date_to` > \''.pSQL($date_from).'\' AND `id`!='.(int) $id_feature_price);
            }
        }
                        ...
                        if ($roomTypeRatesData['roomType']) {
                            foreach ($roomTypeRatesData['roomType'] as $key => $roomTypeRates) {
                                ...
                                if (isset($roomTypeRates['inventory'])) {
                                                        ...
                                                        if (!($objRoomDisableDates->checkIfRoomAlreadyDisabled($params))) {
                                                            if ($numDisabledRooms > 0) {
                                                                ...
                                                                if ($hotelRoomInformation->id_status == 3) {
                                                                    ...
                                                                    if (!$objRoomDisableDates->updateDisableDateRanges($params)) {
                                                ...
                                                if ($disabledRooms) {
                                                    foreach ($disabledRooms as $k_rm => $disableRoom) {
                                                        if ($roomsToEnable > 0) {
                                                            ...
                                                            $params['date_from'] = $dateFrom;
                                                            $params['date_to'] = $dateTo;
                                                            if (!$objRoomDisableDates->deleteDisabledDatesForDateRange($params)) {
                                                            ...
                                                            $disabledDates = $objRoomDisableDates->getRoomDisableDates($disableRoom['id']);
                                    ...
                                    $this->errors[] = $this->moduleInstance->l('Requested rooms inventory is not available.', 'HotelRoomTypeFeaturePricing');
                                }
                            }
                        } else {
            ...
            for ($date = strtotime($date_from); $date <= (strtotime($date_to)-$totalDaySeconds); $date = ($date+$totalDaySeconds)) {
                ...
                if ($featurePrice) {
                    if ($featurePrice['impact_type'] == 1) {
                        //percentage
                        $featureImpactPriceTE = Tools::convertPrice($productPriceTE * ($featurePrice['impact_value'] / 100), $id_currency);
                        $featureImpactPriceTI = Tools::convertPrice($productPriceTI * ($featurePrice['impact_value'] / 100), $id_currency);
                    } else {
                        //Fixed Price
