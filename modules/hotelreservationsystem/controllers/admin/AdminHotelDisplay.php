...
class AdminHotelDisplayController extends ModuleAdminController
{
    ...
    public function renderForm()
    {
        ...
        if ($this->hotelList) {
            ...
            if ($hotelInfos = $this->object->getHotelBlockData($this->context->language->id)) {
                $idHotel = $this->object->id;
                $idExistingHotels = array_column($hotelInfos, 'id_hotel');
                $hotelList = array_filter($hotelList, function ($item) use ($idExistingHotels, $idHotel) {
                    if (!in_array($item['id_hotel'], $idExistingHotels) || ($idHotel && $idHotel != $item['id_hotel'])) {
                        return 1;
                    }
                    return 0;
                });
        ...
        if (!$this->object->id && empty($hotelList)) {
            $this->warnings[] = $this->l('No hotel found to create new hotel display block.');
            return;
        }

        $this->fields_form = array(
            'legend' => array(
                'title' => $this->l('Hotel display configuration'),
                'icon' => 'icon-globe'
            ),
            'input' => array(
                array(
        ...
        if (is_array($this->boxes) && !empty($this->boxes)) {
            foreach ($this->boxes as $id) {
                if (Validate::isLoadedObject($object = new HotelDisplayBlock((int) $id))) {
                    if (!$object->active) {
                        if (Validate::isLoadedObject($objHotelBranchInfo = new HotelBranchInformation($object->id_hotel))) {
                            if (!$objHotelBranchInfo->active) {
