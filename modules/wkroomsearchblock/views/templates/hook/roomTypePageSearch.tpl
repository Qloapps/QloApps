				...
				<div class="row">
					...
					<div class="form-group wk-padding-5 {if isset($location_enabled) && $location_enabled}col-md-5{/if} col-sm-6 {if count($hotels_info) <= 1 && !$show_hotel_name} hidden {/if}">
						{if !$show_hotel_name}
							...
							<input type="hidden" id="preperation_time" name="preperation_time" value="{if isset($hotels_info[0]['preperation_time'])}{$hotels_info[0]['preperation_time']|escape:'htmlall':'UTF-8'}{/if}">
						...
						{else}
							{if isset($hotels_info) && count($hotels_info)}
								<div class="dropdown">
									...
									<input type="hidden" id="preperation_time" name="preperation_time" value="{if isset($preperation_time)}{$preperation_time|escape:'htmlall':'UTF-8'}{/if}">
									...
									<ul class="dropdown-menu hotel_dropdown_ul">
										{if isset($hotels_info) && $hotels_info}
											{foreach $hotels_info as $name_val}
												<li tabindex="-1" class="search_result_li" data-id-hotel="{$name_val['id']|escape:'htmlall':'UTF-8'}" data-hotel-cat-id="{$name_val['id_category']|escape:'htmlall':'UTF-8'}" data-max_order_date="{$name_val['max_order_date']}" data-preperation_time="{$name_val['preperation_time']|escape:'htmlall':'UTF-8'}">{$name_val['hotel_name']|escape:'htmlall':'UTF-8'}</li>
											{/foreach}
										{/if}
									</ul>
								</div>
