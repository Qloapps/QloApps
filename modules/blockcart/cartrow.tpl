...
<dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}_{if isset($hotel_wise_data.id_hotel) && $hotel_wise_data.id_hotel}{$hotel_wise_data.id_hotel}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} pb-1 pt-3 pr-2 pl-3 position-relative bg-shadow">
    <a class="cart-images float-left mr-3" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}"><img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'cart_default')}" alt="{$product.name|escape:'html':'UTF-8'}" /></a>
    <div class="cart-info overflow-hidden pr-4 position-relative">
        {block name='blockcart_shopping_cart_product_name'}
            ...
            {* <!-- quantity changed for number of rooms -->
                ...
                <a class="cart_block_product_name text-white small" href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">{$product.name|truncate:30:'...':true|escape:'html':'UTF-8'}</a>
            </div>
        {/block}
        ...
        {block name='blockcart_shopping_cart_product_total_price'}
            <div class="cart-info-sec small rm_product_info_{$product.id_product}">
                <span class="product_info_label text-grey">{l s='Price' mod='blockcart'}:</span>
                <span class="price product_info_data text-secondary-light ">
        ...
        {block name='blockcart_shopping_cart_product_quantity'}
            <div class="cart-info-sec text-secondary-light small rm_product_info_{$product.id_product}">
                {if $product.allow_multiple_quantity || $product.booking_product}<span class="product_info_label text-grey">{l s='Total Qty.' mod='blockcart'}:</span>{/if}
    ...
    <span class="remove_link position-absolute right-10 top-20 mr-1">
        {if !isset($customizedDatas.$productId.$productAttributeId) && (!isset($product.is_gift) || !$product.is_gift)}
            <a class="ajax_cart_block_remove_link text-white" href="{$link->getPageLink('cart', true, NULL, "delete=1&id_product={$product.id_product|intval}&ipa={$product.id_product_attribute|intval}&id_address_delivery={$product.id_address_delivery|intval}&token={$static_token}")|escape:'html':'UTF-8'}{if !empty($hotel_wise_data.id_hotel)}&id_hotel={$hotel_wise_data.id_hotel|escape:'html':'UTF-8'}{/if}" rel="nofollow" title="{l s='remove this product from my cart' mod='blockcart'}"><i class="icon-circle-xmark"></i></a>
        {/if}
    </span>
    ...
    {if $product.booking_product}
        {block name='blockcart_shopping_cart_dates'}
            <div id="booking_dates_container_{$product.id_product}" class="cart_prod_cont">
                <table class="table text-nowrap table-noborder text-white small mb-0">
                    <tbody>
                        <tr>
