...
<dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}_{if isset($hotel_wise_data.id_hotel) && $hotel_wise_data.id_hotel}{$hotel_wise_data.id_hotel}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} qlo-cart-item">
    <div class="qlo-cart-item__row">
        <a class="qlo-cart-item__image" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">
            <img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'cart_default')}" alt="{$product.name|escape:'html':'UTF-8'}" />
        </a>
        <div class="qlo-cart-item__main">
            <div class="qlo-cart-item__title-row">
                {block name='blockcart_shopping_cart_product_name'}
                    <a class="qlo-cart-item__name" href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">
                        {$product.name|truncate:30:'...':true|escape:'html':'UTF-8'}
                    </a>
                {/block}
                {if !isset($customizedDatas.$productId.$productAttributeId) && (!isset($product.is_gift) || !$product.is_gift)}
                    <a class="ajax_cart_block_remove_link qlo-cart-item__remove" href="{$link->getPageLink('cart', true, NULL, "delete=1&id_product={$product.id_product|intval}&ipa={$product.id_product_attribute|intval}&id_address_delivery={$product.id_address_delivery|intval}&token={$static_token}")|escape:'html':'UTF-8'}{if !empty($hotel_wise_data.id_hotel)}&id_hotel={$hotel_wise_data.id_hotel|escape:'html':'UTF-8'}{/if}" rel="nofollow" title="{l s='remove this product from my cart' mod='blockcart'}">
                        <i class="icon-circle-xmark" aria-hidden="true"></i>
                    </a>
                {/if}
            </div>
            ...
            {block name='blockcart_shopping_cart_hotel_name'}
                {if $product.selling_preference_type == Product::SELLING_PREFERENCE_HOTEL_STANDALONE || $product.selling_preference_type == Product::SELLING_PREFERENCE_HOTEL_STANDALONE_AND_WITH_ROOM_TYPE}
                    <div class="qlo-cart-item__subtext">
            ...
            {block name='blockcart_shopping_cart_product_total_price'}
                <div class="qlo-cart-item__meta">
                    <span class="qlo-cart-item__label">{l s='Price' mod='blockcart'}:</span>
                    <span class="price qlo-cart-item__value">
                        {if !isset($product.is_gift) || !$product.is_gift}
                            ...
                            {/if}
                            <span id="hookDisplayProductPriceBlock-price">
            ...
            {block name='blockcart_shopping_cart_product_quantity'}
                {if $product.allow_multiple_quantity || $product.booking_product}
                    <div class="qlo-cart-item__meta">
                        <span class="qlo-cart-item__label">{l s='Total Qty.' mod='blockcart'}:</span>
                        <span class="qlo-cart-item__value">
                            <span class="quantity">
                                {if $product.booking_product}
                                    {$product.bookingData['total_num_rooms']}
    ...
    {if $product.booking_product}
        {block name='blockcart_shopping_cart_dates'}
            <div id="booking_dates_container_{$product.id_product}" class="cart_prod_cont">
                <table class="cart-booking-table">
                    <tbody>
                        <tr>
                        ...
                        {foreach from=$product.bookingData['date_diff'] key=data_k1 item=data_v}
                            <tr class="rooms_remove_container">
                                ...
                                <td class="qlo-cart-line-remove">
                                    <a class="remove_rooms_from_cart_link" href="#" rm_price="{$data_v['amount']}" id_product="{$product.id_product|intval}" date_from="{$data_v['data_form']}" date_to="{$data_v['data_to']}" num_rooms="{$data_v['num_rm']}" title="{l s='remove this room from my cart' mod='blockcart'}">
                    ...
                    <tbody>
                        ...
                        {foreach from=$options item=data_v}
                            <tr class="product_option_row">
                                ...
                                <td class="text-right">
                                    <a class="ajax_remove_product_option" href="#" id_product="{$product.id_product|intval}" id_hotel="{$data_v.id_hotel|intval}" id_product_option="{$data_v.id_product_option|intval}" title="{l s='remove this product from my cart' mod='blockcart'}">
                                        <i class="icon-trash" aria-hidden="true"></i>
                                    </a>
                                </td>
                            </tr>
