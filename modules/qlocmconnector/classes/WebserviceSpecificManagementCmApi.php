...
class WebserviceSpecificManagementCmApi implements WebserviceSpecificManagementInterface
{
    ...
    public const API_BOOKING_STATUS_MODIFIED = 'modified';
    ...
    protected $error_msg = '';
    protected $id_hotel;
    protected $currentBookingId = null;
                    ...
                    case 'booking_notification':
                        try {
                            ...
                            if ($inputData['booking_status'] == self::API_BOOKING_STATUS_NEW
                                || !Validate::isLoadedObject(new Order($inputData['id_booking']))
                            ) {
                            ...
                            if ($this->currentBookingId) {
                                try {
                                    $this->getBookingDetails($this->currentBookingId);
                                } catch (\Throwable $inner) {
                                    // getBookingDetails() itself failed too — CM still expects the "data" shape,
                                    // so build the minimal version by hand rather than switching response format.
                                    $this->output['data'] = ['id_pms_booking' => $this->currentBookingId];
                                }
                                $this->output['data']['error'] = $objQloCMConnector->l($e->getMessage());
                                $this->getResponseJson();
                                // no exit here — same as the success path, $this->output is picked up
                                // and echoed by the webservice framework after manage() returns.
                            } else {
                                $errorResponse = ["success" => false, "error" => $objQloCMConnector->l($e->getMessage())];
                                echo json_encode($errorResponse, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                                exit;
                            }
                        }
    ...
    public function validateFilters($filters = [])
    {
        ...
        foreach ($filters as $key => $value) {
            if (is_array($value)) {
                ...
                if (!isset($allowedFilters[$key]) || ($key == 'id_room_type' && $filters[$key] == '0')) {
                    $errorResponse = ["success" => false, "error" => $objQloCMConnector->l("Invalid filter: $key")];
                    echo json_encode($errorResponse, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                    exit;
                }
     ...
     */
    public function validateRequestedRoomTypes($roomTypes = array(), $idHotel = false, $idOrder = false)
    {
        if (!$roomTypes) {
            return false;
        }
        ...
        foreach ($roomTypes as $roomType) {
            if ($this->validateRoomType($roomType)) {
                ...
                } elseif ($idHotel != $roomTypeInfo['id_hotel']) {
                    $this->error_msg = sprintf(
                        Tools::displayError('Room type %d does not belong to property %d.'),
                ...
                if (($hotelRoomData = $objBookingDetail->dataForFrontSearch($bookingParams))
                    ...
                    if ($hotelRoomData['stats']['num_avail'] < $roomType['number_of_rooms']) {
                        if(!$this->checkForPariallyAvailableRooms(array(
                            'date_from' => $dateFrom,
                            'date_to' => $dateTo,
                            'hotel_id' => $idHotel,
                            'id_room_type' => $roomType['id_room_type'],
                            'number_of_rooms' => $roomType['number_of_rooms'],
                            'occupancy' => $occupancy
                        ), $this->context->cart->id, $idOrder)) {
                            return false;
                        }
                    } elseif (isset($roomType['rooms']) && count($roomType['rooms'])) {
                        foreach ($roomType['rooms'] as $room) {
                            if (isset($room['id_room'])) {
                                // Validate that the room belongs to the correct hotel
                                $objRoom = new HotelRoomInformation($room['id_room']);
                                if (Validate::isLoadedObject($objRoom) && $objRoom->id_hotel != $idHotel) {
                                    $this->error_msg = Tools::displayError('Room does not belong to the specified hotel.');
                                    return false;
                                }
                                if (!isset($hotelRoomData['rm_data'][$roomType['id_room_type']]['data']['available'][$room['id_room']])) {
                                    return false;
                                }
                            }
                        }
                    ...
                    if ($quantityWise) {
                        ...
                        if (isset($roomType['rooms']) && count($roomType['rooms'])) {
                                foreach ($roomType['rooms'] as $room) {
                                    $child_ages = isset($room['occupancy']['children_ages']) ? $room['occupancy']['children_ages'] : (isset($room['occupancy']['child_ages']) ? $room['occupancy']['child_ages'] : array());

                                    // default age for missing ones
                                    $child_ages = array_pad($child_ages, isset($room['occupancy']['children']) ? $room['occupancy']['children'] : 0, 17);

                                    $occupancy[] = array(
                                        'adults' => isset($room['occupancy']['adults']) ? $room['occupancy']['adults'] : 0,
                                        'children' => isset($room['occupancy']['children']) ? $room['occupancy']['children'] : 0,
                    ...
                    if(!($partiallyAvailableRooms = $this->checkForPariallyAvailableRooms(array(
                        'date_from' => $dateFrom,
                        'date_to' => $dateTo,
                        'hotel_id' => $idHotel,
                        'id_room_type' => $roomType['id_room_type'],
    ...
    {
        QloCMConnector::$idBookingCm = isset($params['id_channel_manager_booking']) && $params['id_channel_manager_booking']
            ? $params['id_channel_manager_booking']
            : uniqid('CM_API_', true);
        ...
        if (!$this->addRoomsInCart($params['room_bookings'])) {
            QloCMConnector::$idBookingCm = null;
            $this->deleteWsServices();
            $this->deleteWsFeaturePrices();
            $this->deleteWsCartRules();
        ...
        if ($objPaymentModule->validateOrder(
            ...
            $this->currentBookingId = (int) $objPaymentModule->currentOrder;
            $this->updateServicesInOrder($objPaymentModule->currentOrder);
            ...
            $this->recalculateOrderFromComponents($objPaymentModule->currentOrder);
            ...
            if(QcmcSourceHelper::coreClassesExist()) {
                $objSourceHelper = new QcmcSourceHelper();
                $objOrder->id_source = $objSourceHelper->getBookingSourceId($params['source'], $params['business_source']);
            }
            ...
            foreach ($roomsInOrder as $orderRoomKey => $orderRoom) {
                ...
                if (isset($this->wsRequestedRooms[$dateRoomJoinKey])) {
                    ...
                    if (isset($this->wsRequestedRooms[$dateRoomJoinKey]['id_tax_rules_group'])) {
                        ...
                        Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'order_tax_detail` WHERE id_order_detail='.(int)$objOrderDetail->id.' AND id_htl_booking='.(int)$objHotelBookingDetail->id);
                    }
    ...
    // order_detail_tax was replaced by order_tax_detail (scoped per room/service via id_htl_booking /
    // id_service_product_order_detail); pass exactly one of $idHtlBooking/$idServiceProductOrderDetail.
    public function saveTaxCalculator($idOrderDetail, $idTaxRulesGroup, $idHtlBooking = 0, $idServiceProductOrderDetail = 0)
    {
        ...
        Db::getInstance()->execute(
            'DELETE FROM `' . _DB_PREFIX_ . 'order_tax_detail`
             WHERE id_order_detail = ' . $idOrderDetail . '
               AND id_htl_booking = ' . $idHtlBooking . '
               AND id_service_product_order_detail = ' . $idServiceProductOrderDetail
        );

        if (empty($this->wsTaxRulesGroup[$idTaxRulesGroup])) {
            return false;
        }
        ...
        foreach ($this->wsTaxRulesGroup[$idTaxRulesGroup] as $taxRuleGroup) {
            $idTax = (int) $taxRuleGroup['tax'];
            $unitAmount = Tools::ps_round(($objOrderDetail->unit_price_tax_excl * $taxRuleGroup['tax_rate']) / 100, _PS_PRICE_COMPUTE_PRECISION_);
            $totalAmount = Tools::ps_round(($objOrderDetail->total_price_tax_excl * $taxRuleGroup['tax_rate']) / 100, _PS_PRICE_COMPUTE_PRECISION_);
            $values[] = '(' . (int) $objOrderDetail->id_order . ', ' . $idOrderDetail . ', ' . $idHtlBooking . ', '
                . $idServiceProductOrderDetail . ', ' . $idTax . ', ' . $unitAmount . ', ' . $totalAmount . ', NOW())';
        }
        ...
        if($taxes) {
            foreach($taxes as $tax) {
                ...
                foreach (Language::getLanguages(false) as $language) {
                    $objTax->name[$language['id_lang']] = substr($tax['name'], 0, 30);
                }
     ...
     */
    public function updateServicesInOrder($idOrder)
    {
        $objOrder = new Order($idOrder);
        $objServiceProductOrderDetail = new ServiceProductOrderDetail();
        if (isset($this->wsRequestedRoomTypes) && $this->wsRequestedRoomTypes) {
            ...
            if ($orderedRooms = $objHotelBookingDetail->getOrderCurrentDataByOrderId($objOrder->id)) {
                foreach ($orderedRooms as $orderedRoom) {
                    ...
                    if(isset($this->wsRequestedRoomTypes[$dateRoomJoinKey]['room']) && $this->wsRequestedRoomTypes[$dateRoomJoinKey]['room']) {
                        $objHotelBookingDetail = new HotelBookingDetail((int)$orderedRoom['id']);
                        $objHotelBookingDetail->rate_plan_name = isset($this->wsRequestedRoomTypes[$dateRoomJoinKey]['room']['rate_plan_name']) ? $this->wsRequestedRoomTypes[$dateRoomJoinKey]['room']['rate_plan_name'] : 'Not Available';
                        $objHotelBookingDetail->save();
                    }
                }
            }
    ...
    /**
     * Recalculates the order total from its actual stored components.
     * This is the SINGLE SOURCE OF TRUTH for order pricing.
     * Used by BOTH booking creation and modification flows.
     *
     * Uses core classes ONLY - no raw SQL queries.
     *
     * Components:
     * - Base price + Taxes: from HotelBookingDetail (rooms)
     * - Services: from ServiceProductOrderDetail
     * - Discounts: from OrderCartRule
     *
     * @param int $idOrder
     * @return float The recalculated total paid tax incl
     */
    public function recalculateOrderFromComponents($idOrder)
    {
        $objOrder = new Order($idOrder);
        $objHotelBookingDetail = new HotelBookingDetail();
        $objServiceProductOrderDetail = new ServiceProductOrderDetail();

        // 1. Get room totals from HotelBookingDetail (active bookings only)
        $roomsTotalTaxIncl = 0;
        $roomsTotalTaxExcl = 0;
        $activeBookingIds = array();
        $activeBookingKeys = array();
        if ($bookings = $objHotelBookingDetail->getBookingDataByOrderId($idOrder)) {
            foreach ($bookings as $booking) {
                if (!$booking['is_refunded'] && !$booking['is_cancelled']) {
                    $roomsTotalTaxIncl += (float) $booking['total_price_tax_incl'];
                    $roomsTotalTaxExcl += (float) $booking['total_price_tax_excl'];
                    $activeBookingIds[] = $booking['id'];
                    $activeBookingKeys[$booking['id']] = true;
                }
            }
        }

        // 2. Get service totals from ServiceProductOrderDetail (active bookings only)
        $servicesTotalTaxIncl = 0;
        $servicesTotalTaxExcl = 0;
        if ($services = $objServiceProductOrderDetail->getServiceProductsInOrder($idOrder)) {
            foreach ($services as $service) {
                if (isset($service['id_htl_booking_detail'])
                    && isset($activeBookingKeys[$service['id_htl_booking_detail']])
                ) {
                    $servicesTotalTaxIncl += (float) $service['total_price_tax_incl'];
                    $servicesTotalTaxExcl += (float) $service['total_price_tax_excl'];
                }
            }
        }

        // 3. Get discount totals from Order cart rules
        $totalDiscountsTaxIncl = (float) $objOrder->getCartRulesTotal(true);
        $totalDiscountsTaxExcl = (float) $objOrder->getCartRulesTotal(false);

        // 4. Calculate final totals
        $totalProductsTaxIncl = $roomsTotalTaxIncl + $servicesTotalTaxIncl;
        $totalProductsTaxExcl = $roomsTotalTaxExcl + $servicesTotalTaxExcl;
        $totalPaidTaxIncl = Tools::ps_round($totalProductsTaxIncl - $totalDiscountsTaxIncl, _PS_PRICE_COMPUTE_PRECISION_);
        $totalPaidTaxExcl = Tools::ps_round($totalProductsTaxExcl - $totalDiscountsTaxExcl, _PS_PRICE_COMPUTE_PRECISION_);

        // 5. Update order totals
        $objOrder->total_products = $totalProductsTaxExcl;
        $objOrder->total_products_wt = $totalProductsTaxIncl;
        $objOrder->total_paid_tax_excl = $totalPaidTaxExcl;
        $objOrder->total_paid_tax_incl = $totalPaidTaxIncl;
        $objOrder->total_paid = $totalPaidTaxIncl;

        // If payment is completed, total_paid_real should match
        if ($objOrder->current_state == Configuration::get('PS_OS_PAYMENT_ACCEPTED')) {
            $objOrder->total_paid_real = $totalPaidTaxIncl;
        }
    ...
    /**
     * Adjusts the order price when the external total_price_with_tax differs
     * from the calculated total. Uses cart rules for differences rather than
     * modifying room/service base prices.
     *
     * @param int $idOrder
     * @param array $params
     */
    ...
    {
        $objOrder = new Order($idOrder);
        if (isset($params['price_details']['total_price_with_tax']) && (float) $params['price_details']['total_price_with_tax']) {
            if ((float) $params['price_details']['total_price_with_tax'] < Tools::ps_round($objOrder->total_paid_tax_incl, _PS_PRICE_COMPUTE_PRECISION_)) {
                $cartRule['code'] = Tools::passwdGen(8, 'NO_NUMERIC');
                $cartRule['currency'] =  '';
                $cartRule['value'] = $objOrder->total_paid_tax_incl - (float) $params['price_details']['total_price_with_tax'];
            ...
            } else if ((float) $params['price_details']['total_price_with_tax'] > Tools::ps_round($objOrder->total_paid_tax_incl, _PS_PRICE_COMPUTE_PRECISION_)) {
                $difference = (float) $params['price_details']['total_price_with_tax'] - Tools::ps_round($objOrder->total_paid_tax_incl, _PS_PRICE_COMPUTE_PRECISION_);
                if ($difference > 0) {
                    $roomsInOrder = (new HotelBookingDetail())->getOrderCurrentDataByOrderId($idOrder);
                    if (empty($roomsInOrder)) {
                        $roomsInOrder = array();
                    }
                    ...
                    if ($firstRoomInOrder) {
                        ...
                        $service['total_price_without_tax'] = $difference;
                        $service['total_price_with_tax'] = $difference;
                        $service['total_tax'] = 0;
                        $service['price_mode'] = self::API_SERVICE_PRICE_MODE_PER_BOOKING;
                        $service['quantity'] = 1;
                        $service['is_new'] = 1;
                        $servicesToAdd = array($service);
                        $this->addServicesInOrderedRoom($servicesToAdd, $firstRoomInOrder['id']);
                    }
                }
            }
        }
    ...
    {
        $this->currentBookingId = (int) $params['id_booking'];
        ...
        if($params['booking_status'] != self::API_BOOKING_STATUS_CANCELLED
            ...
            if ($roomsInOrder = $objHotelBookingDetail->getOrderCurrentDataByOrderId($objOrder->id)) {
                // Detect split rooms: group order rooms by room type to identify splits
                $orderRoomsByType = array();
                foreach ($roomsInOrder as $orderRoomKey => $orderRoom) {
                    $orderRoomsByType[$orderRoom['id_product']][$orderRoomKey] = $orderRoom;
                }

                foreach ($roomsInOrder as $orderRoomKey => $orderRoom) {
                    $requestedRoomTypeKey = $orderRoom['id_product'].'_'.strtotime($orderRoom['date_from']).strtotime($orderRoom['date_to']);
                    if (!isset($roomTypes[$requestedRoomTypeKey])
                        || !isset($roomTypes[$requestedRoomTypeKey]['number_of_rooms'])
                        || $roomTypes[$requestedRoomTypeKey]['number_of_rooms'] <= 0
                    ) {
                        $requestedRoomTypeKey = false;
                        $room_key = 'r_'.$orderRoom['id_room'];
                        foreach ($roomTypes as $roomTypeKey => $roomType) {
                            if ((int) $roomType['id_room_type'] != (int) $orderRoom['id_product']) {
                                continue;
                            }

                            if (!isset($roomType['number_of_rooms']) || $roomType['number_of_rooms'] <= 0) {
                                continue;
                            }

                            if (isset($roomType['rooms'][$room_key])) {
                                $requestedRoomTypeKey = $roomTypeKey;
                                break;
                            }
                            // Fallback for same room type without room-wise mapping.
                            if ($requestedRoomTypeKey === false) {
                                $requestedRoomTypeKey = $roomTypeKey;
                            }
                        }
                    }
                    ...
                    // Detect if this is a split room: multiple order rooms with same room type but different dates
                    $isSplitRoom = false;
                    if (isset($orderRoomsByType[$orderRoom['id_product']])) {
                        $sameTypeRooms = $orderRoomsByType[$orderRoom['id_product']];
                        if (count($sameTypeRooms) > 1) {
                            $firstRoom = reset($sameTypeRooms);
                            if ($firstRoom['date_from'] !== $orderRoom['date_from']
                                || $firstRoom['date_to'] !== $orderRoom['date_to']
                            ) {
                                $isSplitRoom = true;
                            }
                        }
                    ...
                    $requestedCheckIn = date('Y-m-d', strtotime($roomTypes[$requestedRoomTypeKey]['check_in_date']));
                    $requestedCheckOut = date('Y-m-d', strtotime($roomTypes[$requestedRoomTypeKey]['check_out_date']));
                    $room = array();
                    // if there are multiple rooms then there can be a room at index 1 so we are adding a prifix for the below condition.
                    $room_key = 'r_'.$orderRoom['id_room'];
                    if (isset($roomTypes[$requestedRoomTypeKey]['rooms'][$room_key])) {
                        $room = $roomTypes[$requestedRoomTypeKey]['rooms'][$room_key];
                        unset($roomTypes[$requestedRoomTypeKey]['rooms'][$room_key]);
                    } elseif (isset($roomTypes[$requestedRoomTypeKey]['rooms'][0])) {
                        $room = array_shift($roomTypes[$requestedRoomTypeKey]['rooms']);
                    }

                    if ($room) {
                        if ($isSplitRoom) {
                            // For split rooms, preserve segment dates and distribute price proportionally
                            $segmentCheckIn = date('Y-m-d', strtotime($orderRoom['date_from']));
                            $segmentCheckOut = date('Y-m-d', strtotime($orderRoom['date_to']));
                            $room['check_in_date'] = $segmentCheckIn;
                            $room['check_out_date'] = $segmentCheckOut;
                            $room['date_from'] = $segmentCheckIn;
                            $room['date_to'] = $segmentCheckOut;

                            // Calculate proportional price for this split segment
                            $requestedTotalPriceTaxExcl = 0;
                            if (isset($room['unit_price_without_tax'])) {
                                $requestedTotalPriceTaxExcl = $room['unit_price_without_tax'];
                            } else if (isset($room['total_price_with_tax']) && isset($room['total_tax'])) {
                                $requestedTotalPriceTaxExcl = abs($room['total_price_with_tax'] - $room['total_tax']);
                            }
                            $requestedTotalTax = isset($room['total_tax']) ? floatval($room['total_tax']) : 0;

                            // Calculate total nights across all split segments for this room type
                            $totalSplitNights = 0;
                            foreach ($sameTypeRooms as $splitSegment) {
                                $totalSplitNights += HotelHelper::getNumberOfDays($splitSegment['date_from'], $splitSegment['date_to']);
                            }
                            if (!$totalSplitNights) {
                                $totalSplitNights = 1;
                            }

                            $segmentNights = HotelHelper::getNumberOfDays($orderRoom['date_from'], $orderRoom['date_to']);
                            if (!$segmentNights) {
                                $segmentNights = 1;
                            }

                            // Distribute price proportionally by nights
                            $segmentPriceTaxExcl = Tools::ps_round(
                                ($requestedTotalPriceTaxExcl / $totalSplitNights) * $segmentNights,
                                _PS_PRICE_COMPUTE_PRECISION_
                            );
                            $segmentTax = Tools::ps_round(
                                ($requestedTotalTax / $totalSplitNights) * $segmentNights,
                                _PS_PRICE_COMPUTE_PRECISION_
                            );

                            $room['unit_price_without_tax'] = $segmentPriceTaxExcl;
                            $room['total_price_with_tax'] = $segmentPriceTaxExcl + $segmentTax;
                            $room['total_tax'] = $segmentTax;
                        } else {
                            $room['check_in_date'] = $requestedCheckIn;
                            $room['check_out_date'] = $requestedCheckOut;
                            $room['date_from'] = $requestedCheckIn;
                            $room['date_to'] = $requestedCheckOut;
                        }
                        $roomsInOrder[$orderRoomKey]['requested_check_in_date'] = $room['check_in_date'];
                        $roomsInOrder[$orderRoomKey]['requested_check_out_date'] = $room['check_out_date'];
                        // update the room only if there is room wise breakdown.
                        $roomsToUpdate[$requestedRoomTypeKey]['requested'][$orderRoom['id_room']] = $room;
                        $roomsToUpdate[$requestedRoomTypeKey]['order'][$orderRoom['id_room']] = $roomsInOrder[$orderRoomKey];
                    } else {
                        $roomsInOrder[$orderRoomKey]['requested_check_in_date'] = $requestedCheckIn;
                        $roomsInOrder[$orderRoomKey]['requested_check_out_date'] = $requestedCheckOut;
                        $roomsToUpdate[$requestedRoomTypeKey]['order'][$orderRoom['id_room']] = $roomsInOrder[$orderRoomKey];
                    }

                    // For split rooms, don't decrement number_of_rooms since multiple segments
                    // share the same requested room slot
                    if (!$isSplitRoom) {
                        $roomTypes[$requestedRoomTypeKey]['number_of_rooms']--;
                        if ($roomTypes[$requestedRoomTypeKey]['number_of_rooms'] <= 0) {
                            unset($roomTypes[$requestedRoomTypeKey]);
                        }
                    }
            ...
            if (!$this->validateRequestedRoomTypes($params['room_bookings'], $params['id_property'], $params['id_booking'])) {
                QloCMConnector::$idBookingCm = null;
                throw new WebserviceException(
                    ($this->error_msg ?: Tools::displayError('Requested room(s) not available')),
                    array(404, 400)
            ...
            $this->resolveRoomExtensionAvailability($roomsToUpdate, $roomsToAdd, $objOrder->id);
        }

        // Adding new rooms in the booking
        if (count($roomsToAdd)) {
            if (!$this->addRoomsInOrder($objOrder->id, $roomsToAdd)) {
                QloCMConnector::$idBookingCm = null;
                throw new WebserviceException(
                    ($this->error_msg ?: Tools::displayError('Requested room(s) not available')),
                    array(404, 400)
            ...
            foreach ($roomsToUpdate as $roomsByDate) {
                if (isset($roomsByDate['requested']) && $roomsByDate['requested']) {
                    foreach ($roomsByDate['requested'] as $roomsKey => $room) {
                        ...
                        $requestedCheckInDate = isset($room['check_in_date'])
                            ? date('Y-m-d', strtotime($room['check_in_date']))
                            : date('Y-m-d', strtotime($roomsByDate['order'][$roomsKey]['date_from']));
                        $requestedCheckOutDate = isset($room['check_out_date'])
                            ? date('Y-m-d', strtotime($room['check_out_date']))
                            : date('Y-m-d', strtotime($roomsByDate['order'][$roomsKey]['date_to']));
                        $currentCheckInDate = date('Y-m-d', strtotime($roomsByDate['order'][$roomsKey]['date_from']));
                        $currentCheckOutDate = date('Y-m-d', strtotime($roomsByDate['order'][$roomsKey]['date_to']));

                        $isPriceChanged = isset($room['total_price_with_tax'])
                            && ((float) $room['total_price_with_tax']) != ((float) $roomsByDate['order'][$roomsKey]['total_price_tax_incl']);
                        $isDateChanged = ($requestedCheckInDate !== $currentCheckInDate)
                            || ($requestedCheckOutDate !== $currentCheckOutDate);
                        ...
                        // Populate wsRequestedRoomTypes with service data for consistent pricing
                        if (isset($room['services']) && $room['services']) {
                            $this->wsRequestedRoomTypes[$dateRoomJoinKey]['services'] = $room['services'];
                        }
        ...
        if ($params['booking_status'] != self::API_BOOKING_STATUS_CANCELLED && $params['booking_status'] != self::API_BOOKING_STATUS_REFUNDED) {
            ...
            $this->recalculateOrderFromComponents($objOrder->id);
        } else {
            ...
            if ($objOrderState) {
                ...
                if (!in_array($objOrder->current_state, $statuses) && in_array($objOrderState->id, $statuses)) {
                    if ($bookings = $objHotelBookingDetail->getOrderCurrentDataByOrderId($objOrder->id)) {
                            ...
                            foreach ($bookings as $booking) {
                                ...
                                if(Configuration::get('PS_OS_REFUND') == $objOrderState->id){
                                    if(Configuration::get('PS_OS_AWAITING_REMOTE_PAYMENT') == $objOrder->current_state) {
                                        $htlBookingStatusField = 'is_cancelled';
                                    } else {
                                        if ($objOrder->is_advance_payment
                                            ...
                                            $htlBookingStatusField = 'is_refunded';
                                        } else {
                                            $htlBookingStatusField = 'is_cancelled';
                                        }
                                    }
                                ...
                                $objHtlBooking->save();
                                // Not in HotelBookingDetail's definition, so save() drops it.
                                Db::getInstance()->update(
                                    'htl_booking_detail',
                                    array($htlBookingStatusField => 1),
                                    '`id` = '.(int) $objHtlBooking->id
                                );
                        ...
                        // Queue inventory ARI for all room types in the cancel request.
                        // The hook covers current (post-reallocation) DB room types;
                        // this covers original CM-side room types that may differ after reallocation.
                        if (!empty($params['room_bookings']) && is_array($params['room_bookings'])) {
                            $objAriUpdates = new QcmcAriUpdates();
                            $objHotelRoomType = new HotelRoomType();
                            foreach ($params['room_bookings'] as $roomBooking) {
                                if (!isset($roomBooking['id_room_type'])) {
                                    continue;
                                }
                                $idRoomType = (int)$roomBooking['id_room_type'];
                                $roomTypeInfo = $objHotelRoomType->getRoomTypeInfoByIdProduct($idRoomType);
                                if (!$roomTypeInfo) {
                                    continue;
                                }
                                $idHotel = (int)$roomTypeInfo['id_hotel'];
                                $dateFrom = date('Y-m-d', strtotime($roomBooking['check_in_date']));
                                $dateTo = date('Y-m-d', strtotime($roomBooking['check_out_date']));
                                $objAriUpdates->saveAriUpdateRow($idHotel, $idRoomType, $dateFrom, $dateTo);
                            }
                        }
                    }
                }
    ...
    public function updateRoomPriceInOrder($room, $bookingData)
    {
        ...
        if (Validate::isLoadedObject($objHotelBookingDetail = new HotelBookingDetail((int) $idHotelBooking))) {
            ...
            if ($roomPriceTaxExcl) {
                $featureDateFrom = isset($room['check_in_date']) ? $room['check_in_date'] : $room['date_from'];
                $featureDateTo = isset($room['check_out_date']) ? $room['check_out_date'] : $room['date_to'];
                ...
                $this->wsFeaturePrices[] = $this->createFeaturePrice(
                    array(
                        ...
                        'date_from' => $featureDateFrom,
                        'date_to' => $featureDateTo,
            ...
            if (!isset($room['date_from']) && isset($room['check_in_date'])) {
                $room['date_from'] = $room['check_in_date'];
            }
            if (!isset($room['date_to']) && isset($room['check_out_date'])) {
                $room['date_to'] = $room['check_out_date'];
            }
            if (!isset($room['check_in_date']) && isset($room['date_from'])) {
                $room['check_in_date'] = $room['date_from'];
            }
            if (!isset($room['check_out_date']) && isset($room['date_to'])) {
                $room['check_out_date'] = $room['date_to'];
            }
            ...
            $objHotelBookingDetail->date_from = $room['check_in_date'];
            $objHotelBookingDetail->date_to = $room['check_out_date'];
        ...
        if ($services) {
            ...
            foreach ($services as $serviceKey => &$service) {
                if (!isset($service['id_product'])) {
                    ...
                    $servicePriceTaxExculded = floatval($service['total_price_with_tax']) - floatval($service['total_tax']);
                    $serviceTaxRate = (floatval($service['total_tax'])/floatval($servicePriceTaxExculded))*100;
            ...
            foreach ($services as $service) {
                $objServiceProductCartDetail->addServiceProductInCart(
                    ...
                    $service['quantity'],
                    false,
                    isset($roomHtlCartInfo['id']) ? $roomHtlCartInfo['id'] : false
                );
            }
    ...
    public function removeRoomLineFromOrder($params, $roomsToRemove)
    {
        ...
        foreach ($roomsToRemove as $roomType) {
            ...
            $objOrder->total_paid = Tools::ps_round($objOrder->total_paid - ($diffProductsTaxIncl + $additionlServicesTI), _PS_PRICE_COMPUTE_PRECISION_);
            $objOrder->total_paid_tax_incl = Tools::ps_round($objOrder->total_paid_tax_incl - ($diffProductsTaxIncl + $additionlServicesTI), _PS_PRICE_COMPUTE_PRECISION_);
            $objOrder->total_paid_tax_excl = Tools::ps_round($objOrder->total_paid_tax_excl - ($diffProductsTaxExcl + $additionlServicesTE), _PS_PRICE_COMPUTE_PRECISION_);
        ...
        foreach ($roomTypes as &$roomType) {
            ...
            if (isset($roomType['rooms']) && count($roomType['rooms'])) {
                foreach ($roomType['rooms'] as &$room) {
                    ...
                    // add default age if the age if any of the missing
                    $child_ages = array_pad($child_ages, isset($room['occupancy']['children']) ? $room['occupancy']['children'] : 0, 17);
                    ...
                    if (($hotelRoomData = $objBookingDetail->dataForFrontSearch($bookingParams))
                        ...
                        $roomDayFragments = array(); // id_room => list of ['date_from' => .., 'date_to' => ..], one per day
                            ...
                            foreach ($availableRooms['data'] as $key => $availableRoom) {
                                if($key == 'partially_available') {
                                    foreach ($availableRoom as $avlRoom) {
                                        ...
                                        $roomDayFragments[$avl_room['id_room']][] = array(
                                            'date_from' => $avlRoom['date_from'],
                                            'date_to' => $avlRoom['date_to'],
                                        );
                                    }
                                }
                            }
                        ...
                        // Merge contiguous day-fragments per room into continuous date ranges, so a room
                        // that has a gap in availability and later becomes available again still gets
                        // booked as one continuous range instead of fragmenting into multiple entries.
                        $partialRooms = array();
                        foreach ($roomDayFragments as $idRoom => $fragments) {
                            ...
                            foreach ($fragments as $fragment) {
                                if ($merged !== null && $merged['date_to'] == $fragment['date_from']) {
                                    $merged['date_to'] = $fragment['date_to'];
                                    continue;
                                }
                                if ($merged !== null) {
                                    $partialRooms[$idRoom.'_'.$merged['date_from'].'_'.$merged['date_to']] = array(
                        ...
                        if($partialRooms) {
                            // Calculate the requested total price and tax for proportional distribution
                            $requestedTotalPriceTaxExcl = 0;
                            if (isset($room['unit_price_without_tax'])) {
                                $requestedTotalPriceTaxExcl = $room['unit_price_without_tax'];
                            } else if (isset($room['total_price_with_tax']) && isset($room['total_tax'])) {
                            ...
                            $requestedTotalTax = isset($room['total_tax']) ? floatval($room['total_tax']) : 0;
                            $totalSplitNights = 0;
                            foreach ($partialRooms as $splitRoom) {
                                $totalSplitNights += HotelHelper::getNumberOfDays($splitRoom['date_from'], $splitRoom['date_to']);
                            }
                            if (!$totalSplitNights) {
                                $totalSplitNights = 1;
                            }
                            $remainingPriceTaxExcl = Tools::ps_round($requestedTotalPriceTaxExcl, _PS_PRICE_COMPUTE_PRECISION_);
                            $remainingTax = $requestedTotalTax;
                            $processedSplitRooms = 0;
                            $totalSplitRooms = count($partialRooms);
                            ...
                            foreach ($partialRooms as $avlRoom) {
                                if ($idHtlCartBookingData = $objHotelCartBookingData->updateCartBooking(
                                    ...
                                    // Calculate proportional price for this split segment
                                    $processedSplitRooms++;
                                    $segmentNights = HotelHelper::getNumberOfDays($avlRoom['date_from'], $avlRoom['date_to']);
                                    if (!$segmentNights) {
                                        $segmentNights = 1;
                                    }

                                    // Distribute price proportionally by nights, give remainder to last segment
                                    if ($processedSplitRooms == $totalSplitRooms) {
                                        $segmentPriceTaxExcl = $remainingPriceTaxExcl;
                                        $segmentTax = $remainingTax;
                                    } else {
                                        $segmentPriceTaxExcl = Tools::ps_round(
                                            ($requestedTotalPriceTaxExcl / $totalSplitNights) * $segmentNights,
                                            _PS_PRICE_COMPUTE_PRECISION_
                                        ...
                                        $remainingPriceTaxExcl -= $segmentPriceTaxExcl;
                                        $remainingTax -= $segmentTax;
                                    }

                                    // Create feature price for this split room segment
                                    if ($segmentPriceTaxExcl) {
                                        $room['unit_price_without_tax'] = $segmentPriceTaxExcl;
                                        $this->wsFeaturePrices[] = $this->createFeaturePrice(
                                            array(
                                                'id_product' => (int) $roomType['id_room_type'],
                                                'id_cart' => (int) $this->context->cart->id,
                                                'id_guest' => (int) $this->context->cart->id_guest,
                                                'date_from' => date('Y-m-d', strtotime($avlRoom['date_from'])),
                                                'date_to' => date('Y-m-d', strtotime($avlRoom['date_to'])),
                                                'id_room' => $objCartBookingData->id_room,
                                                'price' => $segmentPriceTaxExcl
                                            )
                                        );
                                    }

                                    $dateRoomJoinKey = strtotime($avlRoom['date_from']).strtotime($avlRoom['date_to']).$objCartBookingData->id_product.$objCartBookingData->id_room;
                                    // To update the price after valiate order is called.
                                    $this->wsRequestedRoomTypes[$dateRoomJoinKey]['services'] = $roomServices;
                                    $this->wsRequestedRoomTypes[$dateRoomJoinKey]['room'] = $room;

                                    // Store the split segment tax for later use
                                    if (isset($room['id_tax_rules_group']) && Validate::isLoadedObject(new TaxRulesGroup((int) $room['id_tax_rules_group']))) {
                                        $this->wsRequestedRooms[$dateRoomJoinKey]['id_tax_rules_group'] = $room['id_tax_rules_group'];
                                    } elseif ($segmentTax) {
                                        $this->wsRequestedRooms[$dateRoomJoinKey]['total_tax'] = $segmentTax;
                ...
                $roomServices = array();
                if (!$objHotelCartBookingData->updateCartBooking(
                    $roomType['id_room_type'],
                    $roomWiseOccupancy,
                    'up',
                    $idHotel,
    ...
    public function createWsService(array &$service, $idRoomType, $idServiceTaxRulesGroup = 0)
    {
    	...
    	$price = floatval($service['total_price_with_tax']) - floatval($service['total_tax']);
    ...
    public function addRoomsInOrder($idOrder, $roomTypes)
    {
        ...
        $objRoomType = new HotelRoomType();
        $this->createCartForOrder($objOrder->id);
        if (!$this->addRoomsInCart($roomTypes, $objOrder->id_address_tax)) {
            return false;
        }
        $idOrderInvoice = false;
        if (($objOrderInvoice = $objOrder->getInvoicesCollection()->getFirst())
        ...
        foreach ($roomTypes as $roomType) {
            if (isset($processedRoomTypeIds[$roomType['id_room_type']])) {
                continue;
            }
            $processedRoomTypeIds[$roomType['id_room_type']] = true;
    ...
    public function validatePutRequest($params)
    {
        ...
        if (!isset($params['id_booking']) || !$params['id_booking']) {
            ...
            $this->error_msg = Tools::displayError('Invalid address provided.');
        } elseif (!$this->validatePutRequestRoomTypes($params['room_bookings'], $params['id_property'])
            && $this->error_msg == ''
        ) {
            $this->error_msg = Tools::displayError('Requested room(s) not available');
        }
     ...
     */
    public function validatePutRequestRoomTypes($roomTypes, $idHotel = false)
    {
        $objRoomType = new HotelRoomType();
        foreach ($roomTypes as $roomType) {
            if ($this->validateRoomType($roomType)) {
                // Validate that the room type belongs to the correct hotel
                if ($idHotel) {
                    $roomTypeInfo = $objRoomType->getRoomTypeInfoByIdProduct($roomType['id_room_type']);
                    if ($idHotel != $roomTypeInfo['id_hotel']) {
                        $this->error_msg = Tools::displayError('Room type does not belong to the specified hotel.');
                        return false;
                    }
                }
                ...
                if (isset($roomType['rooms']) && count($roomType['rooms'])) {
                    foreach ($roomType['rooms'] as $room) {
                        ...
                        // Validate that the room belongs to the correct hotel
                        if (isset($room['id_room']) && isset($roomType['id_hotel'])) {
                            $objHotelRoom = new HotelRoomInformation($room['id_room']);
                            if (Validate::isLoadedObject($objHotelRoom) && $objHotelRoom->id_hotel != $roomType['id_hotel']) {
                                $this->error_msg = Tools::displayError('Room does not belong to the specified hotel.');
                                return false;
                            }
                        }
    ...
    {
        $rawFirstname = (isset($inputData['guest_detail']['firstname']) && is_string($inputData['guest_detail']['firstname']))
            ? $inputData['guest_detail']['firstname']
            : '';
        $sanitizedFirstname = trim(preg_replace('/[0-9!<>,;?=+()@#"°{}_$%:]/u', '', stripslashes($rawFirstname)));
        if ($sanitizedFirstname !== '' && mb_strlen($sanitizedFirstname) > 30) {
            $sanitizedFirstname = mb_substr($sanitizedFirstname, 0, 30);
        }
        $inputData['guest_detail']['firstname'] = (Validate::isName($sanitizedFirstname) && $sanitizedFirstname !== '') ? $sanitizedFirstname : 'QloApps';

        $rawLastname = (isset($inputData['guest_detail']['lastname']) && is_string($inputData['guest_detail']['lastname']))
            ? $inputData['guest_detail']['lastname']
            : '';
        $sanitizedLastname = trim(preg_replace('/[0-9!<>,;?=+()@#"°{}_$%:]/u', '', stripslashes($rawLastname)));
        if ($sanitizedLastname !== '' && mb_strlen($sanitizedLastname) > 30) {
            $sanitizedLastname = mb_substr($sanitizedLastname, 0, 30);
        }
        $inputData['guest_detail']['lastname'] = (Validate::isName($sanitizedLastname) && $sanitizedLastname !== '') ? $sanitizedLastname : $inputData['guest_detail']['firstname'];
        ...
        $guestEmail = (isset($inputData['guest_detail']['email']) && is_string($inputData['guest_detail']['email']))
            ? trim($inputData['guest_detail']['email'])
            : '';
        if ($inputData['booking_status'] != self::API_BOOKING_STATUS_NEW && $inputData['id_booking']) {
        ...
        } else if (empty($guestEmail) || mb_strlen($guestEmail) > 128 || !Validate::isEmail($guestEmail)) {
            $inputData['guest_detail']['email'] = Tools::strtolower(trim($inputData['guest_detail']['firstname']));
            $inputData['guest_detail']['email'] .= Tools::strtolower(trim($inputData['guest_detail']['lastname']));
            $inputData['guest_detail']['email'] .= Tools::passwdGen(3, 'NUMERIC').'@qloapps.com';
        }
    ...
    public function addOrderPayment($params)
    {
        if (isset($params['price_details']['total_price_with_tax']) && $params['price_details']['total_price_with_tax'] ) {
            ...
            $objOrder->save();
            $this->managePaymentHistory($objOrder->reference, $objOrder->total_paid_real);
            ...
            foreach ($roomBookings['rooms'] as &$room) {
                if(isset($room['services']) && !empty($room['services'])){
                    ...
                    foreach ($room['services'] as $service) {
                        $totalServicePrice += floatval($service['total_price_with_tax']);
                        if(isset($service['total_tax'])){
                            $totalServiceTax += floatval($service['total_tax']);
                        }
                    }
    ...
    public function checkForPariallyAvailableRooms($bookingParams, $idCart, $idOrder=false)
    {
        $objBookingDetail = new HotelBookingDetail();
        $isPartiallyAvailable = false;
        $isAvailable = false;
        $params = array(
            'date_from' => $bookingParams['date_from'],
            'date_to' => $bookingParams['date_to'],
            'hotel_id' => $bookingParams['hotel_id'],
            'id_room_type' => $bookingParams['id_room_type'],
            'num_rooms' => $bookingParams['number_of_rooms'],
            'search_available' => isset($bookingParams['search_available']) ? $bookingParams['search_available'] : 0,
            'search_partial' => 1,
            'search_booked' => 0,
            'search_unavai' => 0,
            'id_cart' => $idCart ? $idCart : $this->context->cart->id,
            'id_guest' => null,
            'search_cart_rms' => 0,
            'occupancy' => $bookingParams['occupancy'],
            'id_order' => $idOrder
        );

        $roomTypeAvailability = $objBookingDetail->getBookingData($params);
        if($roomTypeAvailability['stats']) {
            if(isset($roomTypeAvailability['stats']['num_part_avai'])) {
                $isPartiallyAvailable = $roomTypeAvailability['stats']['num_part_avai'] ? true : false;
            }
            if(isset($roomTypeAvailability['stats']['num_avail'])) {
                $isAvailable = $roomTypeAvailability['stats']['num_avail'] ? true : false;
            }
        }
        return $isPartiallyAvailable || $isAvailable;
    }

    function getFreeSegments($reqFrom, $reqTo, $bookFrom, $bookTo)
    {
        $segments = [];

        if ($reqFrom < $bookFrom) {
            $beforeEnd = min($reqTo, $bookFrom);

            if ($beforeEnd > $reqFrom) {
                $segments[] = [
                    'date_from' => $reqFrom,
                    'date_to'   => $beforeEnd
                ];
        ...
        if ($reqTo > $bookTo) {
            $afterStart = max($reqFrom, $bookTo);

            if ($reqTo > $afterStart) {
                $segments[] = [
                    'date_from' => $afterStart,
                    'date_to'   => $reqTo
                ];
            }
        }
    ...
    /**
     * How far a specific physical room stays contiguously free, scanning night by
     * night from $anchorDate toward $limitDate (limit may be later or earlier than
     * anchor, supporting both tail- and head-extensions). Delegates to the core
     * dataForFrontSearch() per night so real bookings, disable-dates and LOS
     * restrictions are all honored the same way the rest of the system honors them.
     *
     * Returns $anchorDate unchanged if blocked on the very first night, $limitDate
     * if every night is free, or the boundary in between.
     */
    public function findSameRoomAvailableUntil($idRoom, $idRoomType, $idHotel, $idOrder, $anchorDate, $limitDate)
    {
        $anchorTs = strtotime(date('Y-m-d', strtotime($anchorDate)));
        $limitTs  = strtotime(date('Y-m-d', strtotime($limitDate)));
        if ($limitTs == $anchorTs) {
            return date('Y-m-d', $anchorTs);
        }

        $forward = $limitTs > $anchorTs;
        $step    = $forward ? '+1 day' : '-1 day';
        $objBookingDetail = new HotelBookingDetail();
        $cursor = $anchorTs;

        while ($cursor != $limitTs) {
            $next = strtotime($step, $cursor);
            $dateFrom = date('Y-m-d', min($cursor, $next));
            $dateTo   = date('Y-m-d', max($cursor, $next));

            $data = $objBookingDetail->dataForFrontSearch(array(
                'date_from'        => $dateFrom,
                'date_to'          => $dateTo,
                'hotel_id'         => $idHotel,
            ...
            $roomFreeThisNight = isset($data['rm_data'][$idRoomType]['data']['available'][$idRoom]);
            if (!$roomFreeThisNight) {
                return date('Y-m-d', $cursor);
            }
            $cursor = $next;
        }

        return date('Y-m-d', $limitTs);
    }

    /**
     * Pure validation + in-memory resolution pass for handlePutRequest(). Must run
     * AFTER $roomsToUpdate/$roomsToAdd are fully built and validateRequestedRoomTypes()
     * has passed, and BEFORE addRoomsInOrder()/the roomsToUpdate mutation loop run.
     * Makes zero HotelBookingDetail/OrderDetail/Order writes itself.
     *
     * For every already-assigned room whose requested date range is a genuine
     * extension (per getFreeSegments()), determines how far the SAME physical room
     * can cover the extension via findSameRoomAvailableUntil(). Caps that room's own
     * update to what it can safely cover, and for any uncoverable leftover nights,
     * synthesizes a $roomsToAdd entry (room type, 1 room, no id_room preference) so
     * the existing addRoomsInOrder()/addRoomsInCart() pipeline performs the real
     * availability search + rejection for the leftover. Pure shrinks (no genuine
     * extension on either side) are left untouched — always safe, no check needed.
     */
    public function resolveRoomExtensionAvailability(&$roomsToUpdate, &$roomsToAdd, $idOrder)
    {
        if (!$roomsToUpdate) {
            return;
        }
        ...
        foreach ($roomsToUpdate as &$roomsByDate) {
            if (empty($roomsByDate['requested'])) {
                continue;
            }

            foreach ($roomsByDate['requested'] as $roomKey => &$room) {
                if (!isset($roomsByDate['order'][$roomKey])) {
                    continue;
                }
                $orderRow   = $roomsByDate['order'][$roomKey];
                $idRoom     = (int) $orderRow['id_room'];
                $idRoomType = (int) $orderRow['id_product'];
                $idHotel    = (int) $orderRow['id_hotel'];

                $oldFrom = date('Y-m-d', strtotime($orderRow['date_from']));
                $oldTo   = date('Y-m-d', strtotime($orderRow['date_to']));
                $newFrom = date('Y-m-d', strtotime($room['check_in_date']));
                $newTo   = date('Y-m-d', strtotime($room['check_out_date']));

                $deltaSegments = $this->getFreeSegments($newFrom, $newTo, $oldFrom, $oldTo);
                if (!$deltaSegments) {
                    continue;
                }

                $requestedPriceTaxExcl = isset($room['unit_price_without_tax'])
                    ? $room['unit_price_without_tax']
                    : (isset($room['total_price_with_tax'], $room['total_tax'])
                        ? abs($room['total_price_with_tax'] - $room['total_tax']) : 0);
                $requestedTax    = isset($room['total_tax']) ? floatval($room['total_tax']) : 0;
                $requestedNights = HotelHelper::getNumberOfDays($newFrom, $newTo) ?: 1;

                $cappedFrom = $newFrom;
                $cappedTo   = $newTo;

                foreach ($deltaSegments as $segment) {
                    $isHeadDelta = strtotime($segment['date_from']) < strtotime($oldFrom);
                    $anchorDate  = $isHeadDelta ? $oldFrom : $oldTo;
                    $limitDate   = $isHeadDelta ? $segment['date_from'] : $segment['date_to'];

                    $boundary = $this->findSameRoomAvailableUntil(
                        $idRoom, $idRoomType, $idHotel, $idOrder, $anchorDate, $limitDate
                    );

                    if ($isHeadDelta) {
                        $cappedFrom   = $boundary;
                        $leftoverFrom = $segment['date_from'];
                        $leftoverTo   = $boundary;
                    } else {
                        $cappedTo     = $boundary;
                        $leftoverFrom = $boundary;
                        $leftoverTo   = $segment['date_to'];
                    }

                    if (strtotime($leftoverFrom) < strtotime($leftoverTo)) {
                        $leftoverNights = HotelHelper::getNumberOfDays($leftoverFrom, $leftoverTo) ?: 1;
                        $leftoverPriceTaxExcl = Tools::ps_round(
                            ($requestedPriceTaxExcl / $requestedNights) * $leftoverNights,
                            _PS_PRICE_COMPUTE_PRECISION_
                        );
                        $leftoverTaxAmt = Tools::ps_round(
                            ($requestedTax / $requestedNights) * $leftoverNights,
                            _PS_PRICE_COMPUTE_PRECISION_
                        );

                        $splitKey = $idRoomType.'_'.strtotime($leftoverFrom).strtotime($leftoverTo).'_split_'.$idRoom.'_'.$roomKey;
                        $roomsToAdd[$splitKey] = array(
                            'id_room_type'    => $idRoomType,
                            'number_of_rooms' => 1,
                            'check_in_date'   => $leftoverFrom,
                            'check_out_date'  => $leftoverTo,
                            'rooms' => array(
                                'r_0' => array(
                                    'occupancy'              => isset($room['occupancy']) ? $room['occupancy'] : array(),
                                    'services'               => array(),
                                    'unit_price_without_tax' => $leftoverPriceTaxExcl,
                                    'total_price_with_tax'   => $leftoverPriceTaxExcl + $leftoverTaxAmt,
                                    'total_tax'              => $leftoverTaxAmt,
                                    'taxes'                  => isset($room['taxes']) ? $room['taxes'] : false,
                                ),
                            ),
                        );
                    }
                ...
                if ($cappedFrom !== $newFrom || $cappedTo !== $newTo) {
                    $cappedNights = HotelHelper::getNumberOfDays($cappedFrom, $cappedTo) ?: 1;
                    $cappedPriceTaxExcl = Tools::ps_round(
                        ($requestedPriceTaxExcl / $requestedNights) * $cappedNights,
                        _PS_PRICE_COMPUTE_PRECISION_
                    );
                    $cappedTaxAmt = Tools::ps_round(
                        ($requestedTax / $requestedNights) * $cappedNights,
                        _PS_PRICE_COMPUTE_PRECISION_
                    );

                    $room['check_in_date']  = $cappedFrom;
                    $room['check_out_date'] = $cappedTo;
                    $room['date_from']      = $cappedFrom;
                    $room['date_to']        = $cappedTo;
                    $room['unit_price_without_tax'] = $cappedPriceTaxExcl;
                    $room['total_price_with_tax']   = $cappedPriceTaxExcl + $cappedTaxAmt;
                    $room['total_tax']              = $cappedTaxAmt;
                }
            }
            unset($room);
        }
