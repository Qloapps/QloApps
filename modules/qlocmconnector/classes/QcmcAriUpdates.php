...
class QcmcAriUpdates extends ObjectModel
{
    ...
    //Returns the array of existing ARI update data for a room type, keyed by date
    public function getExistingAriUpdateData($idHotel, $idRoomType)
    {
        $sql = 'SELECT `id_ari_updates` as `id`, `date`, `ari_data`
            FROM `'._DB_PREFIX_.'ari_updates`
            WHERE id_hotel = '.(int)$idHotel.'
            AND id_room_type = '.(int)$idRoomType;

        $existingAriData = Db::getInstance()->executeS($sql);

        // Reindex array by date
        $result = [];
        if ($existingAriData) {
            foreach ($existingAriData as $value) {
                $dateKey = date('Y-m-d', strtotime($value['date']));
    ...
    public function saveAriUpdateRow($idHotel, $idRoomType, $dateFrom, $dateTo, $ariData = [])
    {
        if (empty($idHotel) || empty($idRoomType)) {
            return false;
        }

        // Set default ari_data if not provided
        if(empty($ariData)) {
            $ariData['inventory'] = 1;
        }
        ...
        $ariData = array_merge($defaultAriData, $ariData);

        // Get existing data for this room type, keyed by date
        $getExistingDataByRoomType = $this->getExistingAriUpdateData($idHotel, $idRoomType);

        // for loop from dateFrom to dateTo
        for ($date = $dateFrom; $date <= $dateTo; $date = date('Y-m-d', strtotime($date . ' +1 day'))) {
            if (isset($getExistingDataByRoomType[$date])) {
                // Update existing record
                $objAriUpdates = new QcmcAriUpdates($getExistingDataByRoomType[$date]['id']);
                $existingAriData = json_decode($getExistingDataByRoomType[$date]['ari_data'], true);
                if ($existingAriData) {
                    $ariData = array_merge($existingAriData, $ariData);
                }
            } else {
                // Create new record
            ...
            $objAriUpdates->ari_data = json_encode($ariData);
