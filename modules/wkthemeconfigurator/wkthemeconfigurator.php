...
class WkThemeConfigurator extends Module
{
	...
	public function __construct()
	{
		...
		$this->author = 'Webkul';
		parent::__construct();
		$this->displayName = $this->l('Hotel Theme configurator');
	...
	public function createAjaxController()
	{
		...
		if (is_array($languages))
			foreach ($languages as $language)
				$tab->name[$language['id_lang']] = 'wkthemeconfigurator';
		$tab->class_name = 'AdminWkThemeConfigurator';
	...
	{
		if ($tab_id = (int)Tab::getIdFromClassName('AdminWkThemeConfigurator'))
		{
			$tab = new Tab($tab_id);
			$tab->delete();
		}
	...
	{
		return (
			Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'wkthemeconfigurator`') &&
			Db::getInstance()->Execute('
			CREATE TABLE `'._DB_PREFIX_.'wkthemeconfigurator` (
					...
					`image_h` VARCHAR(10),
					`html` TEXT,
					`content_side` varchar(25),
					`active` tinyint(1) unsigned NOT NULL DEFAULT \'1\',
					PRIMARY KEY (`id_item`)
			) ENGINE = '._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;')
		);//id_product field is added to the table by webkul

	}

	protected function installFixture($hook, $id_image, $id_shop, $id_lang, $id_product, $content_side)
	{
		$result = true;

		$sizes = @getimagesize((dirname(__FILE__).DIRECTORY_SEPARATOR.'img'.DIRECTORY_SEPARATOR.'hotel-room-img'.(int)$id_image.'.png'));
		...
		$result &= Db::getInstance()->Execute('
			INSERT INTO `'._DB_PREFIX_.'wkthemeconfigurator` (
					`id_shop`, `id_lang`, `id_product`, `item_order`, `title`, `title_use`, `hook`, `url`, `target`, `image`, `image_w`, `image_h`, `content_side`, `html`, `active`
			) VALUES (
				\''.(int)$id_shop.'\',
				\''.(int)$id_lang.'\',
				\''.(int)$id_product.'\',
				...
				\'0\',
				\'hotel-room-img'.(int)$id_image.'.png\',
				'.$width.',
				'.$height.',
				\''.$content_side.'\',
				\'\',
	...
	{
		$result = true;
		$side = array(0 => 'left', 1 => 'right', 2 => 'left');
		$all_products = Product::getProducts($this->context->language->id, 0, 3, 'id_product', 'ASC');
		...
		foreach ($languages as $language)
		{
			...
			{
				$result &= $this->installFixture('home', $i, $this->context->shop->id, $language['id_lang'], $value['id_product'], $side[$key]);
				$i++;
			}
	...
	{
		$images = array();
		if (count(Db::getInstance()->executeS('SHOW TABLES LIKE \''._DB_PREFIX_.'wkthemeconfigurator\'')))
			$images = Db::getInstance()->executeS('SELECT image FROM `'._DB_PREFIX_.'wkthemeconfigurator`');
		/*foreach ($images as $image)
			$this->deleteImage($image['image']);*/
		
		if (!Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'wkthemeconfigurator`') || !$this->_removeAjaxContoller() || !parent::uninstall())
			return false;
	...
	{
		$result = $this->getItemsFromHook('home');
		foreach ($result as $key => $value)
		{
			$id_product = $value['id_product'];
			$obj_product = new Product($id_product, false, Configuration::get('PS_LANG_DEFAULT'), $this->context->shop->id, $this->context);
			$result[$key]['product_name'] = $obj_product->name;
			$result[$key]['html'] = $obj_product->description_short;
		}
		$this->context->smarty->assign(array(
			'htmlitems' => $result,
			'hook' => 'home'
	...
	protected function getItemsFromHook($hook)
	{
		...
		SELECT *
		FROM `'._DB_PREFIX_.'wkthemeconfigurator`
	...
	protected function removeItem()
	{
		...
		if ($image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'wkthemeconfigurator` WHERE id_item = '.(int)$id_item))
			$this->deleteImage($image);

		Db::getInstance()->delete(_DB_PREFIX_.'wkthemeconfigurator', 'id_item = '.(int)$id_item);

		if (Db::getInstance()->Affected_Rows() == 1)
		{
			Db::getInstance()->execute('
				UPDATE `'._DB_PREFIX_.'wkthemeconfigurator`
	...
	protected function updateItem()
	{
		...
		$content_side = Tools::getValue('description_side');
		$id_item = (int)Tools::getValue('item_id');
		...
		{
			if ($old_image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'wkthemeconfigurator` WHERE id_item = '.(int)$id_item))
		...
		if (!Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'wkthemeconfigurator` SET
					...
					content_side = \''.pSQL($this->filterVar($content_side), true).'\' 
					WHERE id_item = '.(int)Tools::getValue('item_id')
		))
		{
			if ($image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'wkthemeconfigurator` WHERE id_item = '.(int)Tools::getValue('item_id')))
				$this->deleteImage($image);
	...
	protected function addItem()
	{
		...
		if (!Db::getInstance()->Execute('
			INSERT INTO `'._DB_PREFIX_.'wkthemeconfigurator` (
					`id_shop`, `id_lang`, `id_product`, `item_order`, `title`, `title_use`, `hook`, `url`, `target`, `image`, `image_w`, `image_h`, `html`, `content_side`, `active`
			) VALUES (
	...
	protected function renderThemeConfiguratorForm()
	{
		...
		foreach ($this->languages as $language)
		{
			...
			foreach ($hooks[$language['id_lang']] as $hook)
				$items[$language['id_lang']][$hook] = Db::getInstance()->ExecuteS('
					SELECT * FROM `'._DB_PREFIX_.'wkthemeconfigurator`
		...
		$this->context->smarty->assign('htmlitems', array(
			'items' => $items,
			'theme_url' => $this->context->link->getAdminLink('AdminWkThemeConfigurator'),
			'lang' => array(
		...
		//BY WEBKUL ASSIGNING ALL PRODUCTS LIST TO
